...
use crate::{
    ...
    install_dataset_metadata::InstallMetadataReadError,
};
...
impl ZoneContents {
    ...
    fn expected_result(&self, dir: &Utf8Path) -> ZoneManifestArtifactResult {
        let status = if !self.write_to_disk {
            // Missing from the disk. We have to return a string here, so we
            // just emulate the (hopefully) fixed message.
...
pub(crate) fn dataset_missing_error(
    ...
    InstallMetadataReadError::DatasetDirMetadata {
        ...
        // Error 2 is ENOENT on every platform
        error: ArcIoError::new(io::Error::from_raw_os_error(2)),
    }
}
