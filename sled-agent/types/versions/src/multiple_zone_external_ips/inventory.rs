...
use iddqd::IdOrdItem;
use iddqd::id_upcast;
use omicron_common::api::external::{self, Generation};
...
pub const MAX_ZONE_EXTERNAL_IPS: usize = 8;
...
#[schemars(transparent, length(min = 1, max = MAX_ZONE_EXTERNAL_IPS))]
pub struct NexusExternalIps(Vec<IpAddr>);
...
#[schemars(transparent, length(min = 1, max = MAX_ZONE_EXTERNAL_IPS))]
pub struct ExternalDnsAddrs(Vec<SocketAddr>);
...
impl From<SourceNatConfigGeneric> for ZoneSnatConfig {
    fn from(snat: SourceNatConfigGeneric) -> Self {
        // `first_port`/`last_port` are `pub(crate)`, so we can rebuild the
        // family-typed config directly within this crate.
        match snat.ip {
...
impl From<v14::inventory::OmicronSledConfig> for OmicronSledConfig {
    fn from(v14: v14::inventory::OmicronSledConfig) -> Self {
        Self {
            generation: v14.generation,
            disks: v14.disks,
            datasets: v14.datasets,
            zones: v14.zones.into_iter().map(Into::into).collect(),
            remove_mupdate_override: v14.remove_mupdate_override,
            host_phase_2: v14.host_phase_2,
            measurements: v14.measurements,
        }
    }
}

impl TryFrom<OmicronSledConfig> for v14::inventory::OmicronSledConfig {
    type Error = external::Error;
...
impl TryFrom<ConfigReconcilerInventoryStatus>
    for v14::inventory::ConfigReconcilerInventoryStatus
{
    type Error = external::Error;
...
impl TryFrom<ConfigReconcilerInventory>
    for v16::inventory::ConfigReconcilerInventory
{
    type Error = external::Error;
...
impl TryFrom<Inventory> for v46::inventory::Inventory {
    type Error = external::Error;
