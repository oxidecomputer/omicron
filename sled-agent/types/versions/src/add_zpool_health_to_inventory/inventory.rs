...
impl From<Inventory> for v22::inventory::Inventory {
    fn from(value: Inventory) -> Self {
        ...
        Self {
            ...
            reservoir_size,
            disks,
            zpools: zpools.into_iter().map(Into::into).collect(),
            datasets,
            ledgered_sled_config,
            reconciler_status,
            last_reconciliation,
...
impl From<InventoryZpool> for v1::inventory::InventoryZpool {
    fn from(value: InventoryZpool) -> Self {
        let InventoryZpool { id, total_size, health: _ } = value;
        Self { id, total_size }
    }
}
