...
pub struct ProbeCreate {
    ...
    /// Multicast groups the probe's OPTE port should subscribe to during
    /// zone provisioning. Each entry carries a `group_ip` and an optional
    /// source filter (for SSM), matching the instance multicast join. An
    /// empty list (the default) requests no subscriptions.
    #[serde(default)]
