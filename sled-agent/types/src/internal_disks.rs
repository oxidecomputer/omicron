...
use crate::raw_disks::MaybeUpdatedDisk;
...
impl InternalDisksReceiver {
    ...
    pub fn spawn_internal_disks_task(
        mount_config: Arc<MountConfig>,
    ...
    pub fn borrow_and_update_raw_disks(
        &mut self,
    ...
    /// purposes.
    pub fn errors(&self) -> Arc<BTreeMap<DiskIdentity, DiskError>> {
...
pub struct InternalDisksWithBootDisk {
    inner: InternalDisks,
    boot_disk: DiskIdentity,
}

impl InternalDisksWithBootDisk {
    fn new(inner: InternalDisks) -> Option<Self> {
        let boot_disk = inner.disks.iter().find_map(|d| {
            if d.is_boot_disk() { Some(d.identity.clone()) } else { None }
        })?;
        Some(Self { inner, boot_disk })
    }

    fn boot_disk(&self) -> &InternalDiskDetails {
        let key = InternalDiskDetailsId {
            is_boot_disk: true,
            identity: &self.boot_disk,
        };
        match self.inner.disks.get(&key) {
            Some(details) => details,
            None => unreachable!("boot disk present by construction"),
        }
    }

    pub fn boot_disk_id(&self) -> &DiskIdentity {
        &self.boot_disk().identity
    }
    ...
    pub fn non_boot_disk_install_datasets(
        ...
        self.inner
            .disks
            .iter()
            .filter(|disk| disk.identity != self.boot_disk)
            .map(|disk| {
...
impl IdOrdItem for InternalDiskDetails {
    type Key<'a> = InternalDiskDetailsId<'a>;
    ...
    fn from(disk: &'_ InternalDisk) -> Self {
        let zpool_id = match disk.zpool_name() {
            ...
            ZpoolName::External(_) => {
                panic!("InternalDisk has external ZpoolName: {disk:?}");
            }
        };
...
struct InternalDiskDetailsId<'a> {
    is_boot_disk: bool,
    identity: &'a DiskIdentity,
}

impl cmp::Ord for InternalDiskDetailsId<'_> {
...
impl cmp::PartialOrd for InternalDiskDetailsId<'_> {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

// Wrapper around a `Disk` that ensures statically it is an internal disk.
#[derive(Debug, Clone)]
pub struct InternalDisk(Disk);
    ...
    async fn reconcile_internal_disks<T: DiskAdopter>(
        ...
        self.disks_tx.send_if_modified(|disks| {
            ...
            for new_disk in disks_to_insert {
                disks.insert_overwrite(InternalDisk(new_disk));
                changed = true;
            }
...
mod tests {
    ...
    async fn firmware_updates_are_propagated() {
        ...
        let (raw_disks_tx, raw_disks_rx) =
            watch::channel(Arc::new([raw_disk.clone()].into_iter().collect()));
        let disk_adopter = Arc::new(TestDiskAdopter::default());
        let mut disks_rx = InternalDisksReceiver::spawn_with_disk_adopter(
            Arc::new(any_mount_config()),
            RawDisksReceiver(raw_disks_rx),
    ...
    async fn removed_disks_are_propagated() {
        ...
        let (raw_disks_tx, raw_disks_rx) = watch::channel(Arc::new(
            [&raw_disk1, &raw_disk2].into_iter().cloned().collect(),
        ));
        let disk_adopter = Arc::new(TestDiskAdopter::default());
        let mut disks_rx = InternalDisksReceiver::spawn_with_disk_adopter(
            Arc::new(any_mount_config()),
