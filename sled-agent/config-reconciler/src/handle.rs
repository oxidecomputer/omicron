...
impl AvailableDatasetsReceiver {
    ...
    /// observed it, or `Err` if it can never change again because the
    /// config reconciler task is gone. The test variants' datasets never
    /// change, so for them this never returns.
