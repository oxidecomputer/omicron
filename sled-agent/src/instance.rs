...
impl InstanceRunner {
    ...
    async fn setup_propolis_inner(&mut self) -> Result<PropolisSetup, Error> {
        ...
        if !self
            ...
            .await
            .all_u2_zpools()
            .contains(&self.filesystem_pool)
        {
            return Err(Error::FilesystemPoolNotFound(
                self.filesystem_pool.clone(),
            ));
        }
        let root = self.filesystem_pool.dataset_mountpoint(ZONE_DATASET);
