...
pub async fn spawn_all_longrunning_tasks(
    ...
    let (mut config_reconciler, ledger_task_spawn_token) =
    ...
    let (config_reconciler_spawn_token, ledger_task_run_rx) = config_reconciler
        .spawn_ledger_task(
            SledAgentArtifactStoreWrapper(Arc::clone(&artifact_store)),
            ledger_task_spawn_token,
        );

    // This will only work after we've spawned the ledger task
    // which should happen based on the token progression
    let measurements = Arc::new(
        MeasurementsHandle::new(
            log.clone(),
            config_reconciler.clone(),
            zone_image_resolver.clone(),
            ledger_task_run_rx,
        )
        .await,
    );
...
async fn spawn_trust_quorum_task(
    ...
    let (mut node, handle) =
        trust_quorum::NodeTask::new(config, log, measurements).await;
