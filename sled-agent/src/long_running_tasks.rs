...
pub async fn spawn_all_longrunning_tasks(
    ...
    let cold_boot_measurements = get_cold_boot_measurements(log, &config_reconciler, &zone_image_resolver).await;
...
async fn spawn_trust_quorum_task(
    ...
    let measurements_rx =
        config_reconciler.measurement_corpus_rx(cold_boot_measurements).await.clone();
