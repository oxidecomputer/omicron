...
pub struct EarlyNetworkConfig {
    ...
    // For the purposes of cold boot of inidividual sleds and racks (but notably
    // not individual scrimlets), before update of uplink configuration is
    // possible and driven by an RPW, we store the switch management addresses
    // in the bootstore. This allows us to reuse the exact same values  on the
    // next boot.
    pub switch_mgmt_addrs: HashMap<SwitchLocation, Ipv6Addr>,
}
