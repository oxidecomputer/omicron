...
use nexus_sled_agent_shared::inventory::{
    Inventory, OmicronSledConfig, OmicronZoneConfig, OmicronZonesConfig,
    SledRole,
};
...
impl SledAgent {
    ...
    pub(crate) async fn inventory(&self) -> Result<Inventory, InventoryError> {
        ...
        let omicron_physical_disks_generation = ledgered_sled_config
            .as_ref()
            .map(|sled_config| sled_config.generation)
            .unwrap_or_else(Generation::new);
        let omicron_zones = ledgered_sled_config
            .map(|sled_config| OmicronZonesConfig {
