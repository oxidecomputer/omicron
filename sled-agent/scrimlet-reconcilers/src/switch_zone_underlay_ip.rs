...
impl ThisSledSwitchZoneUnderlayIpAddr {
    ...
    /// `StartSledAgentRequest`. We can't statically enforce this requirement,
    /// but failure to adhere to this convention defeats the entire point of
    /// this type: it is supposed to provide a way for `sled-agent` (and related
    /// crates, like this one) to statically "tag" an IP address as being the IP
    /// of its own, local switch zone.
