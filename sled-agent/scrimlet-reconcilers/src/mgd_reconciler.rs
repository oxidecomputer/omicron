    ...
    async fn do_reconciliation(
        ...
        let routers = match render_desired_routers(
            ...
            Err(err) => {
                ...
                return MgdReconcilerStatus::FailedGeneratingDesiredConfig(err);
            }
        };
...
fn render_uplink_bgp_spec(
    ...
    let BgpConfig { asn: _, originate, shaper, checker, max_paths } =
    ...
    Ok(Some(BgpSpec {
        ...
        checker: checker.clone().map(|code| MgdCheckerSource { asn, code }),
...
fn convert_operator_bgp(bgp: &RouterConfigBgpSpec) -> BgpSpec {
    ...
    BgpSpec {
        ...
        checker: bgp.checker.clone().map(|code| MgdCheckerSource { asn, code }),
...
fn uplink_peer_parameters(peer: &UplinkBgpPeerConfig) -> MgdBgpPeerParameters {
