 ...
 * serializing the object.
 */
pub struct HttpResponseOkOneObject<T: Serialize>(pub T);
impl<T: Serialize> HttpResponse for HttpResponseOkOneObject<T> {}
impl<T: Serialize> From<HttpResponseOkOneObject<T>>
    for Result<Response<Body>, HttpError>
{
    fn from(
        HttpResponseOkOneObject(body_object): HttpResponseOkOneObject<T>,
    ) -> Result<Response<Body>, HttpError> {
