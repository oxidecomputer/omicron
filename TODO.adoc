...
* I've made a mess of stuff though with imports and such. See lots of XXXs.
** new ServerController interface
** add it to my demo tool!
** ServerController startup needs to take a flag which indicates how to
   simulate (auto or explicit)
* the server-controller binary may need to grow a config+logging mechanism
* todo: update / rip out built-in SCs? (may as well rip out built-in projects
  too)
* lots of cleanup needed
* restructure the code:
** server_controller: body, entrypoints, client (for itself)
** controller: body, internal entrypoints, external entrypoints, internal client
   (for itself)
** generic:
*** model (maybe with separate exports for internal model vs. external model?)
*** http_client
*** config? is this specific to controller?
*** error
*** datastore
*** test_util
* steps for the above:
** move clients to their respective places
** create separate exports for internal vs. external models
** rename entrypoints functions
** revisit "Api" prefixes
** review 'use' statements everywhere, and src/lib.rs especially
