...
* implement disks
** create basic model types
** create API endpoints to create (sync?), get, list, delete (sync?)
** create API endpoints to attach, detach
*** probably need a field in the disk that is the instance it's attached to
*** attach/detach can use a mechanism similar to instance_ensure() to load a
    disk on a server controller.
*** might disks want a "faulted" state as well?
*** will probably want a runtime state of the disk that includes bits related to
    attach/destroy.  Cases to handle include:
**** attach while creating
**** destroy while creating, attaching, detaching, or detached
** implement async "creating"?
