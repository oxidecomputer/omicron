...
* done: separate out server controller library-like interfaces
...
** updated: binary needs to be updated
* oxide-controller changes:
** updated: needs an API surface for requests from the server controller.
   Ideally, this would be a separate Dropshot API but we could skip this for now.
** partially done: needs an actual API endpoint for "SC just started up".
*** done: implementation needs to add the SC if it doesn't exist already
*** todo: needs to find instances that should be there and ensure() them
*** todo: needs to find disks that should be there and ensure() them
* test suite needs to be updated to start separate server controllers
  as well as the internal _and_ external servers
