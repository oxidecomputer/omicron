...
=== Currently: plan continuously, execute continuously

By default in production, the planner runs periodically. `omdb` can be used to:
...
The planner background task is enabled by default on real systems but disabled in test environments. It can be controlled via `omdb nexus reconfigurator-config --planner-enabled {true,false}`.
