...
=== Asserting before async setup completes
...
=== Asserting before an async operation completes
...
=== Stale watch channel notifications
...
**How to fix this:** Wait on a predicate over the channel's _value_, not on change notifications. Use `wait_for_watch_channel_condition` with a reasonable timeout  (preferred), or `watch::Receiver::wait_for`. This requires the channel to carry enough state to express "the operation I triggered is done". This could be, for example, a generation number the test compares with `>=` (which also tolerates coalescing).
...
=== Background tasks mutating the SUT
...
=== Database time resolution mismatch
