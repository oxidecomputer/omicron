...
** The followup https://github.com/oxidecomputer/omicron/commit/YYYY[`YYYY`] disables scheduled collections in the Oximeter agent tests entirely, and makes the tests request each collection explicitly. This prevents the bounded channel from ever filling up.
// FIXME: replace YYYY with the merged commit hash of the oximeter agent test fix.
