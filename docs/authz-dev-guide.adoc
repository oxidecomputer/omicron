...
CAUTION: These instructions haven't really been tested since we've never added a _new_ resource with roles.  This is more of a tour of the implementation of a role.
...
6. Define an API endpoint to read and update the policy on the resource.  You can use the existing examples as templates.  The bulk of the implementation is in common code that's generic over the resource type so you shouldn't need to do a lot of work here.

== Defining New Roles

See <<_supporting_roles_on_resources>>.  To define a new role, you'll need to add it to the shared enum type and to the database enum type.  You may also need a schema migration to add the new role name.

The enum definition drives most of the rest of the system, including the set of roles that a user is allowed to set on an object.  You don't need to change any of the endpoints that modify policy.
