...
This document explains how to work with the authorization (authz) subsystem in Nexus. Authorization is based on role-based access control (RBAC) using the Oso policy engine.  Read the module comments in `nexus/auth/src/authz/mod.rs` and `nexus/auth/src/authz/api_resources.rs` to understand the basic concepts.
...
Here, `DiskPath` is the user-provided identifier and corresponds to `{disk}` in the path above (`/v1/disks/{disk}`).

==== App Layer

The details of authz at the app layer depend on the kind of resource.  See the various sections below.  Continuing the example above, `Disk` is a dynamic project-level resource.  You'd wind up having either the `disk_id` already:
...
or else having the project name (or id) and disk name:
...
However this winds up looking for the type of resource you're working with, you'll end up having resolved the user-provided identifier to an `authz` object that can be used for doing authz checks.
...
* `nexus/db-queries/src/policy_test/resource_builder.rs` - Policy test related trait implementations
* `nexus/db-lookup/src/lookup_path.rs` - LookupPath implementation
...
In `nexus/db-queries/src/policy_test/resource_builder.rs`, `DynAuthorizedResource` is implemented for the resource:
...
[source,rust]
----
impl_dyn_authorized_resource_for_resource!(authz::Inventory);
----
