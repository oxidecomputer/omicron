...
//     (I wrote a note about this but don't remember anything more than that.)
...
* From the <<check-blueprint-planner,`blueprint_planner` background task>>.  This shows the planning report for the most recent blueprint the planner generated, _whether or not that blueprint was saved to the database or became the target_.
+
...
If there have been no recent blueprints, **what is the planner currently waiting for?**  <<check-blueprint-planner,Check>> to see.  You will likely see one of the following:
...
* `reconfigurator config not yet loaded; doing nothing`.  This should only happen during Nexus startup and should not last long.  If it does, <<check-background-tasks,check the task status>> for the `reconfigurator_config_watcher` background task.
...
* `no inventory collection available` error.  This should only happen _sometimes_ immediately after an upgrade and before Nexus has managed to collect inventory.  This should not last long.  If it does, <<check-background-tasks,check the task status>> for the `inventory_collection` background task.
...
If you see this in the <<check-blueprint-planner,blueprint planner status>>:
...
XXX `omdb nexus mgs-updates`

[#debug-blocked-MGS-update]
=== Debugging why an MGS-driven update is blocked

// XXX-dap

XXX go through the list of common reasons why it gets blocked -- but this might overlap with stuff that's already in the table above so maybe this isn't necessary
...
XXX check ground truth
XXX check inventory_collection background task
XXX check inventory_loader background task
XXX check blueprint_planner background task
