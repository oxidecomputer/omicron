...
// XXX-dap
// - what you can't do
//   - Set the target release in the middle of an update
//   - Set the target release backwards
...
// - how to verify that everything is working okay
// - customer view
//   - time last step planned
//   - suspended
//   - progress
// - safeties
...
You can thus observe these steps by looking at the blueprint history.
...
3. As the name suggests, the planning report describes what's important about the most recently generated blueprint.  (In this case, that's the blueprint that was thrown away because it was identical to its parent.)
...
The planning report is extremely valuable for understanding either the step that the system just decided to take (if the blueprint _was_ inserted) or why it's currently stuck (if the blueprint wasn't inserted because it's identical to its parent).  In the example above, there's just nothing to do.

During the phase of the upgrade where the system is updating SPs, you might see this:

```
planning report:
* 1 pending MGS update:
  * model0:BRM42220051: RotBootloader(PendingMgsUpdateRotBootloaderDetails { expected_stage0_version: ArtifactVersion("0.0.1"), expected_stage0_next_version: NoValidVersion })
...
- the system has configured an update to the RoT bootloader for sled `BRM42220051`.
- the system is not updating any zones because there are some MGS-driven updates not yet done
- the system is not starting Nexus handoff because there are zones not updated

This is a totally normal thing to see during an update for a few minutes.  If the system got stuck here, though, you'd want to investigate why the RoT bootloader update wasn't completing.
// XXX-dap link
// XXX-dap separate section on planning reports?
