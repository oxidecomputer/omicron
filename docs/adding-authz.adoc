...
Singleton resources representing system-wide concepts or synthetic collections. These are almost always synthetic resources that don't correspond to database entities. Examples: `Fleet`, `Inventory`, `DnsConfig`, `SiloCertificateList`, `VpcList`.
...
Resources with multiple instances that correspond to database entities. This is the most common case. Examples: `Silo`, `IpPool`, `Disk`, `VpcSubnet`.
