...
switch0.address = "[::1]:57997"

# Look for "management gateway:    http://[::1]:44345 (switch1)"
# The "http://" does not go in this string -- just the socket address.
switch1.address = "[::1]:57200"

[deployment.internal_dns]
# Look for "internal DNS:          [::1]:51571"
...
[deployment.database]
# Look for "cockroachdb URL:       postgresql://root@[::1]:64123/omicron?sslmode=disable"
# and adjust the port number below.
url = "postgresql://root@[::1]:64123/omicron?sslmode=disable"
# You should not need to change this.
type = "from_url"
################################################################################
----
