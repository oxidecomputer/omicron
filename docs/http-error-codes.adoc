...
* "503 Service unavailable" is used when the service or a dependency is overloaded or actually unavailable.
...
Some sources suggest using "204 No Content" for PUT endpoints, which is a reasonable choice.  We usually return "200 OK" with the new representation of the resource, which seems more useful.
...
There are also https://httpwg.org/specs/rfc7231.html#rfc.section.6.2[100-level status codes], whose use is generally more standardized and well-understood.

There are also https://httpwg.org/specs/rfc7231.html#status.3xx[300-level status codes], which are pretty well-understood and we don't expect to make heavy use of.
