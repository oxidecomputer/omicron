...
(In this example, every variant happens to carry the same fields, so we could
consider different styles of writing this that avoided this duplication. But
usually different variants have different types of fields or different types of
inner errors, so we'll live with the duplication for simplicity.)
...
provides the `InlineErrorChain` type that:

* Includes the full error chain, separated by `: `, in its `Display` impl
...
in the chain later than the correctly-defined one will be lost.

For example, consider a three deep error chain where the middle error does not
include its source but the outermost error does:
...
error variant can attach meaningful context even without other fields. In cases
where there is truly no meaningful context to attach (e.g., if wrapping an inner
error that already includes all relevant context), consider using
...
strongly-typed error types that can be acted on by callers. If using anyhow, the
above guidance about "attach meaningful context to errors" still applies, but is
easier to do at call sites. Continuing the example from above, we could rewrite
our function to use `anyhow`'s `Context` extension to `Result` and `Option`:
...
chain in a `: `-separated single line (just like `InlineErrorChain`):
