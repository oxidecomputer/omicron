...
=== Verifying that everything is working okay before the update

Before an update, we expect the response of the system update status endpoint to return `contact_support: false`. If viewing only through the console, there should be no warning asking the user to contact support.
...
* about 15-20 minutes after the update is finished, check the system update status endpoint, or the console view of "System Update" to see if the response is telling the user they should call support.
...
Every time the system update status endpoint is called and the `contact_support` boolean is true, the results of the failed health checks will be captured in the logs.
...
- The health checks will run during a MUPdate so the endpoint may show false positives if an MUPdate is happening.
