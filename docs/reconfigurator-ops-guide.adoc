...
=== Verifying that the system is ready to be updated

Before an update, we expect the response of the system update status endpoint to return `contact_support: false`. If viewing only through the console, there should be no warning asking the user to contact support. See <<task-check-for-contact-support-flag>>.

Should the `contact_support` field be true, see <<task-check-cause-of-contact-support-true>>.
...
* a few minutes after the update is finished, check the system update status endpoint, or the console view of "System Update" to see if the response is telling the user they should call support. See <<task-check-for-contact-support-flag>>. If contact support is true see <<task-check-cause-of-contact-support-true>>.

See the previous sections for how to observe these.
...
* During the many brief periods when components or sleds are restarted, some disk create operations may fail, if they get allocated to Crucible instances that are currently offline.  Disk deletion may also fail for disks whose downstairs instance is offline.
...
[#task-check-for-contact-support-flag]
=== Task: Check for contact_support flag

NOTE: This field is intended for update purposes only. It flags whether it is safe to start an update, an update has stopped unexpectedly, or whether health checks on a subset of components passed after an update completed. It is NOT a health status API. If used as a health status API, it could show false positives and false negatives. For example, a newly commissioned sled that hasn't reported a reconciliation result yet would set the contact_support field to true. And many potential problems in the system are _not_ checked in computing this flag.

Use the `oxide system update status` command to see the status of the contact support field

```
$ oxide system update status
{
  "target_release": {
    time_requested: "2026-06-11T05:34:02.158806Z",
    version: "20.0.0-0.ci+git863b79d5cd0"
  },
  "components_by_release_version": {
    "20.0.0-0.ci+git863b79d5cd0": 249
  },
...
If viewing only through the console, the contact support field won't be present. If there is an issue, a warning will appear instead.

image::assets/contact-support-true.png[]

[#task-check-cause-of-contact-support-true]
=== Task: Checking why the contact support field in system update status is true

Every time the "system update status" endpoint is called, the system checks for a small set of known issues. If any are found, it reports `contact_support: true`.
	
Prior to the update, this flag indicates issues that could prevent an update from proceeding normally.
During the update, this flag indicates that the update has stopped and requires support intervention to continue.
After the update, this flag indicates that the update may have resulted in a system that's not fully working.
	
The API doesn't say anything else because this isn't customer-actionable. Details are available in the Nexus log that serviced the API request.

This is an example of how the log entry would look in a very unhealthy system:

```
00:11:54.269Z WARN e6bff1ff-24fb-49dc-a54e-c6a350cd4d6c (dropshot_external): found problems in the system before or after an update
    authenticated = true
    enabled_smf_services_not_online_by_sled = {1cd22575-85e2-471d-88fd-3ec5c5e61ed6 (sled): SvcsEnabledNotOnline(SvcsEnabledNotOnline { services: [SvcEnabledNotOnline { fmri: "svc:/system/test2:default", zone: "global", state: Offline }, SvcEnabledNotOnline { fmri: "svc:/system/test:default", zone: "global", state: Maintenance }], errors: [], time_of_status: 2026-05-11T07:34:08.063055Z })}
    local_addr = 127.0.0.1:12220
    method = GET
    remote_addr = 127.0.0.1:61900
    req_id = 9ed1b600-1963-4691-b400-2a78c0cea4ba
    silo_id = 980314eb-2969-4a13-b407-be9f3d93d330
    silo_user_id = 41278141-e86f-41e8-b398-b0310a62d0ca
...
- If you call the update status endpoint during an update, the health checks won't run and no problems will be reported unless the system considers an update to be stuck.
- There are many conditions outside the context of a self-service update that can transiently cause `contact_support: true`, such as when a sled is rebooting or a MUPdate is in progress. This flag should only be used to determine if it's okay to proceed with a self-service update, if a self-service update is stuck, or if a self-service update appears to have introduced a serious problem.
...
|blueprint_pruner
|Removes old blueprints from the database
...
**Is the blueprint pruner stuck?**  <<task-check-background-tasks,Check the `blueprint_pruner` background task to see if it's reporting errors.>>  If so, resolving these should allow the pruner to continue removing old blueprints, making space for new ones.

If the pruner is not stuck, it's possible that blueprints are being created faster than the pruner is cleaning them up.  **Is the blueprint planner in a loop, creating the same blueprint (or same sequence of blueprints)?**  Check <<task-check-reconfigurator-history,`omdb reconfigurator history`>> to see if the recent blueprints appear to be the same.  For example, you could see dozens of lines like this:
...
In terms of the live system, your options depend on the nature of the problem.  You should generally be able to <<task-disable-planner,disable the planner>> and let the pruner clean up a bunch of old blueprints (or use `omdb reconfigurator archive` to do this yourself).  Then you could re-enable the planner.  However, if it's stuck in a loop, this might not help much.  In that case, you may have to disable the planner, wait for a release with the fix to be available, and MUPdate to it.  Depending on what's causing the planner to generate these blueprints, there may be some other change you can make to the system to stop the planner doing this, or you may be able to <<task-override-plan,override the planner>>.
