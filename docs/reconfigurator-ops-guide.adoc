...
NOTE: This field is intended for update purposes only. It flags whether it is safe to start an update, an update has stopped unexpectedly, or whether health checks on a subset of components passed after an update completed. It is NOT a health status API. If used as a health status API, it could show false positives. For example, a newly commissioned sled that hasn't reported a reconciliation result yet would set the contact_support field to true.
...
- The health checks will run during a MUPdate so the endpoint may show false positives if a MUPdate is happening.
