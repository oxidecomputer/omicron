...
ranges = [ { first = "192.168.1.20", last = "192.168.1.29" } ]
----

This pool contains a range of IP addresses on your external network that Omicron can assign to externally-facing services (like DNS and the API).  You'll need to change the range if you've picked different addresses for your external network.  See <<_external_networking>> above for more on this. You can also add more ranges or additional pools.
...
In a real rack, two of the sleds (referred to as Scrimlets) will be connected directly to the switch (Sidecar). Those sleds will thus be configured with a switch zone (`oxz_switch`) used to manage the switch. The `deployment` table in Sled Agent's config says what kind of system the sled is part of, which decides whether it is a Scrimlet and which switch backend its switch zone runs.

The relevant config will be in `smf/sled-agent/$MACHINE/config.toml`.

[source,text]
----
# For simulated Omicron: one host running a SoftNPU zone; always a scrimlet.
deployment = { kind = "standalone", front_port_count = 1, rear_port_count = 1 }
----

The deployment kinds are:

1. `production`: an Oxide rack. The hardware monitor detects the Tofino ASIC
and the sled runs as a Scrimlet when an ASIC is attached. The sidecar board revision
defaults to `"b"` and can be set with `sidecar_revision`.
2. `virtual`: a propolis-hosted, multi-sled lab such as voxel. `sled-agent` probes for
the propolis SoftNPU device at startup and runs as a Scrimlet when one is attached.
Requires `front_port_count` and `rear_port_count`.
3. `standalone`: a single host running a SoftNPU zone. Always a Scrimlet.
Requires `front_port_count` and `rear_port_count`.
4. `custom`: an explicit `sled_mode` (`auto`, `sled`, or `scrimlet`) and a
`switch` table whose `kind` is `tofino_asic`, `tofino_stub`,
`soft_npu_propolis_device`, or `soft_npu_zone`.

With a custom deployment kind, the detected backends
`tofino_asic` and `soft_npu_propolis_device` take `auto`; the fixed backends
`tofino_stub` and `soft_npu_zone` take `scrimlet`; `sled` ignores the switch.
This gimlet-standalone config is an example:

[source,text]
----
deployment = { kind = "custom", sled_mode = "scrimlet", switch = { kind = "tofino_stub" } }
----

For the purposes of local development, the `soft_npu_zone` switch is used.  Unfortunately, Omicron does not currently automatically configure the switch with respect to external networking, so you'll need to manually do so.
