...
The gateway's MAC address is automatically determined by searching the ARP
table for the given gateway IP. This can be overridden by setting `GATEWAY_MAC`.
These variables are used to configure `softnpu` with a default route for external /
...
=== Set up simulated scrimlet/sidecar dependencies.

To access the outside world or other gimlets, we'll need to set up a virtual sidecar
...
----
$ GATEWAY_IP=10.85.0.1 GATEWAY_MAC=68:d7:9a:1f:77:a1 PHYSICAL_LINK=ixgbe0 pfexec ./tools/create_virtual_hardware.sh
----
