...
/// Fold a reply's round trip time into a target's running statistics.
fn record_rtt(t: &mut Ping4State, dt: Duration) {
    t.current = Some(dt);
    if t.low == Duration::ZERO || dt < t.low {
        t.low = dt;
    }
    if dt > t.high {
        t.high = dt;
...
fn update_lost_packet_count(t: &mut Ping4State) {
    if t.first != 0 {
            ...
            loop {
                ...
                if let Ok((sz, from)) = self.sock.recv_from(&mut ubuf) {
                    ...
                    if let Some(sent) = target.sent {
                        record_rtt(target, Instant::now() - sent);
                    }
                }
            }
        });
