...
struct RunArgs {
    ...
    ///
    /// When any group is present the run creates a multicast IP pool per SSM/ASM
    /// bucket the groups require, enrolls every probe into every group at
    /// probe-create time, and verifies data-plane delivery by pinging each
    /// group and confirming every member replies.
    ///
    ...
    /// required: an any-source join would deliver and there would be nothing to
    /// deny. Repeatable.
    #[arg(
        long,
...
///
/// Sources, when present, must match the group's address family. IPv4-only
/// enforcement lives in `validate_mcast`, which rejects v6 groups across both
/// arg lists. Invoked by the clap value parsers so that a returned `Err` is
...
/// otherwise, in which case the multicast phase is skipped. Each entry carries a
/// group and its (possibly empty) source filter.
#[derive(Debug, Clone)]
struct McastConfig {
    groups: Vec<McastGroup>,
}

/// One multicast IP pool that must exist for a run: its name and the address
/// range covering the groups it backs.
struct McastPool {
    name: &'static str,
    begin: IpAddr,
    end: IpAddr,
...
impl McastConfig {
    ...
    /// would cross 232/8 whenever groups sit on either side of it. The groups
    /// are therefore partitioned into up to three buckets: ASM below the SSM
    /// block, ASM above it, and SSM. Each non-empty bucket yields one pool
    /// whose range is the min..max span over the bucket's group addresses,
    /// which guarantees the pool contains every group it must back while
    /// staying entirely ASM or entirely SSM. Classification uses the canonical
    /// [`is_ssm_address`] to stay in lockstep with Nexus.
    fn pools(&self) -> Vec<McastPool> {
        const SSM_BLOCK_START: IpAddr = IpAddr::V4(Ipv4Addr::new(232, 0, 0, 0));
        const SSM_BLOCK_END: IpAddr =
            IpAddr::V4(Ipv4Addr::new(232, 255, 255, 255));
        [
    ...
    fn bucket(
        ...
        let end = *addrs.iter().max().expect("min implies non-empty");
        Some(McastPool { name, begin, end })
    }
}
...
impl RunArgs {
    ...
    fn validate_mcast(&self) -> Result<(), String> {
        ...
        // - split probe addresses and `McastReports` by v4/v6
...
async fn rack_prepare(
    ...
    if let Some(cfg) = args.mcast() {
        // A multicast pool is single-family and entirely SSM or entirely ASM, so
        // create one pool per SSM/ASM bucket the groups require.
        for pool in cfg.pools() {
            ensure_mcast_pool(&oxide, pool.name, pool.begin, pool.end).await?;
        }
    }
    ...
    api_retry!(
        match oxide.system_ip_pool_view().pool(pool_name).send().await {
            ...
            Err(e) if e.status() == Some(reqwest::StatusCode::NOT_FOUND) => {
                print!(
                    "multicast ip pool {pool_name} does not exist, creating ... "
                );
                oxide
                    .system_ip_pool_create()
                    .body(IpPoolCreate {
...
async fn launch_probes(
    ...
    // multicast pool. Subsequent probes resolve to the same groups and attach as
    // new members.
    //
...
/// One idempotent `ensure_probe` pass: view, membership check, and any needed
/// delete and create.
...
async fn probe_has_memberships(
    ...
    for spec in groups {
        ...
        let Some(member) = members.iter().find(|m| m.parent_id == probe_id)
        else {
            return Ok(false);
        };
...
/// A multicast dataplane report, written to
/// `multicast-connectivity-report.json`.
#[derive(Serialize)]
struct McastReports {
    v4: Vec<McastGroupReport<Ipv4Addr>>,
}

/// Verify multicast replication delivery to subscribed members of every
/// configured group.
///
/// This mirrors `test_connectivity` but pings each multicast group address
/// rather than each probe directly. Every joined member is expected to reply to
/// every group, so the probe addresses are the expected member set and each
/// must produce replies within the loss tolerance. Reports are written to
/// the connectivity report before evaluation, so a failing group's accounting
...
fn test_mcast_connectivity(
    ...
    let warmup: Duration = args.warmup.into();
    let mut reports = McastReports { v4: Vec::new() };
    ...
    // The multicast reply travels back as unicast, so return-path loss shares
    // the unicast reliability and reuses the same tolerance. Single-copy
    // delivery (tx == rx) is enforced separately by the over-delivery check.
...
/// delivery. A dual-homed member that receives one copy per switch echoes twice
/// per request, pushing `rx_count` past `tx_count`. Designated-forwarder
/// election now gates the external ingress entry to a single switch, so steady
/// state is exactly one delivered copy per request. Under-delivery reuses the
/// unicast loss tolerance because the reply returns over the unicast path, while
...
fn verify_mcast_report<T: std::fmt::Display + PartialEq>(
    ...
    // any wire copy of its own stream means the fan-out included a port it
    // must exclude.
    if report.sender_self_rx != 0 {
        panic!(
            "multicast group {group}: sender received {} wire copy(ies) of \
             its own request stream; the dataplane must not replicate a \
             sender's group traffic back toward its ingress port",
...
/// Panic if any member of a source-filtered group received delivery.
///
...
fn verify_mcast_report_deny<T: std::fmt::Display>(report: &McastReport<T>) {
    ...
    if report.sender_self_rx != 0 {
        panic!(
            "multicast group {group}: sender received {} wire copy(ies) of \
             its own request stream; a source-filtered group must not \
             replicate toward the sender's ingress port",
            report.sender_self_rx,
        );
    }
    println!(
