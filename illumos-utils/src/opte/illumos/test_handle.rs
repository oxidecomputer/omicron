...
//! Handle to a dummy, in-memory OPTE driver used in tests.
...
fn opte_state() -> &'static Mutex<State> {
    OPTE_STATE.get_or_init(|| Mutex::new(State { ports: HashMap::new() }))
}
...
impl Handle {
    ...
    pub fn add_router_entry(
        ...
        let Some(PortData { routes, .. }) = inner.ports.get_mut(&req.port_name)
            ...
            return Err(OpteError::InvalidArgument(format!(
                "No such port '{}'",
                req.port_name
            )));
        };
