...
pub enum ProgenitorOperationRetryError<E> {
    ...
    ProgenitorError(#[source] progenitor_client013::Error<E>),
}
...
pub struct ProgenitorOperationRetry<
    ...
    E: std::fmt::Debug,
    F: FnMut() -> Fut,
    Fut: Future<Output = Result<T, progenitor_client013::Error<E>>>,
    BF: FnMut() -> BFut,
        ...
        retry_notify(
            ...
            move || {
                ...
                async move {
                    ...
                    match f.await {
                        Err(progenitor_client013::Error::CommunicationError(e)) => {
                            warn!(
                                log,
                            ...
                            Err(BackoffError::transient(
                                ProgenitorOperationRetryError::ProgenitorError(
                                    progenitor_client013::Error::CommunicationError(e)
                                )
                            ))
                        }

                        Err(progenitor_client013::Error::ErrorResponse(
                            response_value,
                        )) => {
                            match response_value.status() {
                                ...
                                | http::StatusCode::TOO_MANY_REQUESTS => {
                                    Err(BackoffError::transient(
                                        ProgenitorOperationRetryError::ProgenitorError(
                                            progenitor_client013::Error::ErrorResponse(
                                                response_value
                                            )
                                        )
                                    ))
