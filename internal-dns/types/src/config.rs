...
impl DnsConfigBuilder {
    ...
    /// Construct a complete [`v2::DnsConfigParams`] (suitable for propagating to
    /// our DNS servers) for the control plane DNS zone described up to this
    /// point
    pub fn build_full_config_for_initial_generation(
        self,
    ) -> v2::DnsConfigParams {
...
pub mod v1 {
    ...
    impl TryInto<v2::DnsConfigParams> for DnsConfigParams {
        type Error = anyhow::Error;
        fn try_into(self) -> Result<v2::DnsConfigParams, Self::Error> {
            let mut converted_zones: Vec<v2::DnsConfigZone> = Vec::new();
            for zone in self.zones.into_iter() {
                converted_zones.push(zone.try_into()?);
            }

            Ok(v2::DnsConfigParams {
    ...
    // See docs on [`v2::DnsConfigZone`] for more about this struct. They are functionally
    ...
    impl TryInto<v2::DnsConfigZone> for DnsConfigZone {
        type Error = anyhow::Error;
        fn try_into(self) -> Result<v2::DnsConfigZone, Self::Error> {
            let converted_records: HashMap<String, Vec<v2::DnsRecord>> = self
                .records
                .into_iter()
                .filter_map(|(name, name_records)| {
                    let converted_name_records: Vec<v2::DnsRecord> =
                        name_records
                            .into_iter()
                            .map(|rec| match rec {
                                DnsRecord::A(x) => v2::DnsRecord::A(x),
                                DnsRecord::Aaaa(x) => v2::DnsRecord::Aaaa(x),
                                DnsRecord::Srv(x) => {
                                    v2::DnsRecord::Srv(x.into())
                                }
                            })
                            .collect();
...
pub mod v2 {
    ...
    impl TryInto<crate::config::v1::DnsConfig> for DnsConfig {
        type Error = anyhow::Error;
        fn try_into(self) -> Result<crate::config::v1::DnsConfig, Self::Error> {
            panic!("lol")
        }
    }
    ...
    impl From<v1::Srv> for Srv {
        fn from(other: v1::Srv) -> Self {
            Srv {
                prio: other.prio,
                weight: other.weight,
                port: other.port,
