...
pub struct RackNetworkConfigInput {
    ...
    /// The ports whose applied settings feed the bootstore config. A duplicate
    /// `(switch, port_name)` is reported as a [`Problem::DuplicateSwitchPort`]
...
pub enum Problem {
    ...
    /// More than one port shares a `(switch, port_name)`. The database enforces
    /// this uniqueness per rack, but the bootstore read is global for
    /// multi-rack, so a duplicate here is an inconsistency we refuse rather
    /// than emit twice.
    DuplicateSwitchPort { switch: SwitchSlot, port: String },
...
impl fmt::Display for Problem {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Problem::DuplicateSwitchPort { switch, port } => write!(
                f,
                "port {port} on switch {switch:?} appears more than once in \
                 the bootstore config",
            ),
...
pub fn build_rack_network_config(
    ...
    for port in port_inputs {
        // The bootstore read is global for multi-rack, so two racks sharing a
        // port name collide on `(switch, port_name)`. Report the duplicate
        // rather than emit it twice.
        if !seen_ports.insert((port.switch, port.port_name.clone())) {
            problems.push(Problem::DuplicateSwitchPort {
                switch: port.switch,
...
mod tests {
    ...
    #[test]
    fn duplicate_switch_port_is_reported() {
        // The same port name on different switches is distinct; the same
        // (switch, name) is a duplicate, reported rather than emitted twice.
        let err = build_rack_network_config(input_with(
            vec![],
            vec![
                bare_port(SwitchSlot::Switch0, "qsfp0"),
                bare_port(SwitchSlot::Switch1, "qsfp0"),
                bare_port(SwitchSlot::Switch0, "qsfp0"),
            ],
        ))
        .unwrap_err();
        assert_eq!(
            err.problems,
            vec![Problem::DuplicateSwitchPort {
    ...
    fn no_silent_port_drops(
        ...
        if input_names.is_empty() {
            ...
            // the test.
            // The builder iterates ports in input order, so the expected
            // problems follow that order.
