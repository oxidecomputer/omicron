...
impl NexusExternalApi for NexusExternalApiImpl {
    ...
    async fn subnet_pool_subnet_add(
        ...
        let handler = async {
            ...
            let member = nexus
                .subnet_pool_subnet_add(&opctx, &path.pool, &subnet_params)
                .await?;
    ...
    async fn subnet_pool_subnet_remove(
        ...
        let handler = async {
            ...
            let subnet_params = subnet_params.into_inner();
            nexus
                .subnet_pool_subnet_remove(&opctx, &path.pool, &subnet_params)
                .await?;
            Ok(HttpResponseUpdatedNoContent())
        };
        apictx
    ...
    async fn v2025121200_multicast_group_view(
        ...
        let handler = async {
            ...
            let opctx =
                crate::context::op_context_for_external_api(&rqctx).await?;
            let group_selector = params::MulticastGroupSelector {
                multicast_group: path.multicast_group.into(),
            };
            let group = apictx
                .context
                .nexus
    ...
    async fn v2025121200_lookup_multicast_group_by_ip(
        ...
        let handler = async {
            ...
            let opctx =
                crate::context::op_context_for_external_api(&rqctx).await?;
            let group_selector = params::MulticastGroupSelector {
                multicast_group: path.address.into(),
            };
            let group = apictx
                .context
                .nexus
