...
async fn sled_agent_put(
    ...
    let sled_id = &path.sled_id;
    let handler = async {
        nexus.sled.upsert_sled(&opctx, *sled_id, info).await?;
...
async fn cpapi_disks_put(
    ...
    let handler = async {
        ...
        nexus
            .disk
            .notify_disk_updated(&opctx, path.disk_id, &new_state)
            .await?;
        Ok(HttpResponseUpdatedNoContent())
    };
    apictx.internal_latencies.instrument_dropshot_handler(&rqctx, handler).await
...
async fn cpapi_collectors_post(
    ...
    let handler = async {
        ...
        nexus
            .oximeter
            .upsert_oximeter_collector(&opctx, &oximeter_info)
            .await?;
        Ok(HttpResponseUpdatedNoContent())
    };
    context
...
async fn saga_list(
    ...
    let handler = async {
        ...
        let opctx = crate::context::op_context_for_internal_api(&rqctx).await;
        let saga_stream =
            nexus.sec_client().sagas_list(&opctx, &pagparams).await?;
...
async fn saga_view(
    ...
    let handler = async {
        ...
        let saga = nexus
            .sec_client()
            .saga_get(&opctx, steno::SagaId::from(path.saga_id))
            .await?;
...
async fn bgtask_list(
    ...
    let handler = async {
        let nexus = &apictx.nexus;
        let opctx = crate::context::op_context_for_internal_api(&rqctx).await;
        let bgtask_list = nexus.background_tasks().list(&opctx).await?;
...
async fn bgtask_view(
    ...
    let handler = async {
        ...
        let path = path_params.into_inner();
        let bgtask =
            nexus.background_tasks().status(&opctx, &path.bgtask_name).await?;
...
async fn bgtask_activate(
    ...
    let handler = async {
        ...
        nexus
            .background_tasks()
            .activate_bgtasks(&opctx, body.bgtask_names)
            .await?;
        Ok(HttpResponseUpdatedNoContent())
    };
    apictx.internal_latencies.instrument_dropshot_handler(&rqctx, handler).await
...
async fn sled_add(
    ...
    let handler = async {
        let opctx = crate::context::op_context_for_internal_api(&rqctx).await;
        let id = nexus.rack.sled_add(&opctx, sled.into_inner()).await?;
