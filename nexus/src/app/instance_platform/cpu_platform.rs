...
/// The Platonic ideal Milan. This is what we would "like" to define as "The Milan vCPU platform"
/// absent any other constraints. This is a slightly slimmer version of the Milan platform defined
/// in RFD 314, with justifications there.
///
/// Notably, this avoids describing individual processor SKUs' characteristics, where possible.
...
fn milan_ideal() -> CpuIdDump {
    ...
    leaf.set_cflush_cache_line_size(8); // TODO: BREAKING
    ...
    // Leaf 6: Power management and some feature bits. Power management is all zeroed.
    ...
    cpuid.set_extended_topology_info(None);
    ...
    cpuid.set_l2_l3_cache_and_tlb_info(None);
    ...
    cpuid.set_tlb_1gb_page_info(None);
    ...
    cpuid.set_extended_cache_parameters(None);
    ...
    leaf.set_no_nested_data_bp(true); // TODO: BREAKING
    leaf.set_lfence_always_serializing(true); // TODO: BREAKING
    leaf.set_null_select_clears_base(true); // TODO: BREAKING
...
pub fn milan_rfd314() -> Vec<CpuidEntry> {
    ...
    // and it was passed through. The extended space is not supported in Bhyve, but we leave it set
    // here to not change it from under guests.
    //
    // Bhyve now supports all six performance counters, so we could set the perf counter extension
    // bit here, but again it is left as-is to not change CPUID from under a guest.
...
fn milan_rfd314_is_as_described() {
    ...
    assert_eq!(MILAN_CPUID, computed);
}
