...
/// Resolved DPD parameters derived from an external/underlay group pair.
struct ResolvedGroupParams {
    vlan_id: Option<u16>,
...
impl MulticastDataplaneClient {
    ...
    /// Resolve VLAN ID, underlay address, and NAT target from group pair.
    fn resolve_group_params(
        external_group: &ExternalMulticastGroup,
        underlay_group: &UnderlayMulticastGroup,
    ) -> MulticastDataplaneResult<ResolvedGroupParams> {
        ...
        Ok(ResolvedGroupParams { vlan_id, underlay_ip, nat_target })
    }
    ...
    pub(crate) async fn create_groups(
        ...
        let ResolvedGroupParams {
            vlan_id,
            underlay_ip: underlay_ip_admin,
            nat_target,
        } = Self::resolve_group_params(external_group, underlay_group)?;
    ...
    pub(crate) async fn update_groups(
        ...
        let ResolvedGroupParams {
            ...
            underlay_ip: underlay_ip_admin,
            nat_target,
        } = Self::resolve_group_params(
            params.external_group,
            params.underlay_group,
        )?;
