    ...
    fn make_saga_dag(
        ...
        match &params.disk {
            ...
            datastore::Disk::LocalStorage(_) => {
                // Clean up of the local storage resources is now done entirely
                // in the `local_storage_delete` background task. The structure
                // of this saga has been left untouched (to handle the case
                // where Nexus is mupdated to a new version containing this
                // change, but will resume any existing interrupted sagas) but
                // it is now an error to create a new version of this saga for
