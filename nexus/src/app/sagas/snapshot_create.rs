...
async fn ssc_create_snapshot_record(
    ...
    warn!(log, "grabbing disk by name {}", params.create_params.disk);
    ...
    warn!(log, "creating snapshot {} from disk {}", snapshot_id, disk.id());
    ...
    warn!(log, "created snapshot {} ok", snapshot_id);
...
async fn ssc_create_snapshot_record_undo(
    ...
    let snapshot_id = sagactx.lookup::<Uuid>("snapshot_id")?;
    warn!(log, "deleting snapshot {}", snapshot_id);
...
async fn ssc_send_snapshot_request(
    ...
    match disk.runtime().attach_instance_id {
        Some(instance_id) => {
            warn!(log, "disk {} instance is {}", disk.id(), instance_id);
            ...
            warn!(log, "instance {} sled agent created ok", instance_id);
        ...
        None => {
            warn!(log, "disk {} not attached to an instance", disk.id());
...
async fn ssc_send_snapshot_request_undo(
    ...
    for (dataset, region) in datasets_and_regions {
        ...
        warn!(log, "deleting snapshot {} {} {}", url, region.id(), snapshot_id);
        client
...
async fn ssc_start_running_snapshot(
    ...
    for (dataset, region) in datasets_and_regions {
        ...
        warn!(log, "dataset {:?} region {:?} url {}", dataset, region, url);
        ...
        warn!(log, "crucible region {:?}", crucible_region);
        ...
        warn!(log, "crucible snapshot {:?}", crucible_snapshot);
        ...
        warn!(log, "crucible running snapshot {:?}", crucible_running_snapshot);
        ...
        warn!(log, "map {} to {}", region_addr, snapshot_addr);
...
async fn ssc_create_volume_record(
    ...
    warn!(log, "disk volume construction request {}", disk_volume.data());
    ...
    warn!(log, "snapshot volume construction request {}", volume_data);
    ...
    warn!(log, "volume {} created ok", volume_id);
...
async fn ssc_create_volume_record_undo(
    ...
    let osagactx = sagactx.user_data();
    let volume_id = sagactx.lookup::<Uuid>("volume_id")?;
    warn!(log, "deleting volume {}", volume_id);
...
async fn ssc_finalize_snapshot_record(
    ...
    warn!(log, "snapshot final lookup...");
    ...
    warn!(log, "snapshot final lookup ok");
