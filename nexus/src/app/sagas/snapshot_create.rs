...
mod test {
    ...
    impl Nexus {
        pub fn sec_client(&self) -> Arc<steno::SecClient> {
            self.sec_client.clone()
        }
    }

    #[nexus_test]
    async fn test_create_snapshot_record_idempotency(cptestctx: &ControlPlaneTestContext) {
        let client = &cptestctx.external_client;

        // Create silo, org, project, and disk
        let silo = create_silo(&client, "test-silo", false, shared::UserProvisionType::Fixed).await;
        let org = create_organization(&client, "test-org").await;
        let project = create_project(client, "test-org", "test-proj").await;
        let disk = create_disk(&client, "test-org", "test-proj", "disk").await;
        ...
        let saga_params = Arc::new(sagas::snapshot_create::Params {
            ...
            organization_id: org.identity.id,
            ...
            create_params: params::SnapshotCreate {
                ...
                disk: "disk".parse().unwrap(),
            },
        });

        let saga_template = SagaTemplateBuilder::new();

        saga_template.append(
            "snapshot_id",
            "GenerateSnapshotId",
            new_action_noop_undo(saga_generate_uuid),
        );
        ...
        // TODO execute saga then validate results
    }
}
