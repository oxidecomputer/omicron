    ...
    match last_request_step.step_type {
        db::model::RegionReplacementStepType::Propolis => {
            ...
            if current_vmm.id != step_vmm_id {
                ...
                let state = current_vmm.runtime.state.state();
                ...
                match &state {
                    ...
                    InstanceState::Creating | InstanceState::Starting => {
                        // These states are unexpected, considering Nexus
                        // previously sent a target replacement request to this
                        // propolis!

                        return Err(ActionError::action_failed(format!(
                            "vmm {} propolis is Creating or Starting",
                            step_vmm_id,
                        )));
                    }
            ...
            Some(instance_id) => {
                ...
                match instance_and_vmm.vmm() {
                    Some(vmm) => {
                        ...
                        let state = vmm.runtime.state.state();
