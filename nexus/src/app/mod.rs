...
pub struct Nexus {
    ...
    resolver: Arc<Mutex<internal_dns_names::multiclient::Resolver>>,
}
...
impl Nexus {
    ...
    pub async fn set_resolver(
        &self,
        resolver: internal_dns_names::multiclient::Resolver,
    ) {
        *self.resolver.lock().await = resolver;
    }

    pub async fn resolver(&self) -> internal_dns_names::multiclient::Resolver {
