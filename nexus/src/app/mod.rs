...
impl Nexus {
    ...
    pub async fn external_tls_config(
        ...
        // XXX-dap Create a rustls::ServerConfig similar to what Dropshot
        // already constructs, but with a custom cert resolver.  Then, check if
        // we've already read the certs from the database.  If so, return
        // immediately.  If not, activate the background task and wait for that
        // to produce a result.
        todo!(); // XXX-dap
    }
