...
impl super::Nexus {
    ...
    fn activate_router_configuration_propagation(&self) {
        self.background_tasks.activate(
            &self.background_tasks.task_switch_port_settings_manager,
        );
    }
    ...
    pub async fn router_configuration_update(
        ...
        // The fixed (name, switch) of a built-in configuration, if this is
        // one; both are immutable.
...
mod tests {
    ...
    fn duplicate_named_links_keep_the_highest_priority_entry() {
        let named = Uuid::new_v4();
        let list =
            router_list_from_links(vec![(300, named), (100, named)]);
        assert_eq!(
            list,
