...
impl MulticastGroupReconciler {
    ...
    async fn process_joining_reconcile_result(
        ...
        match reconcile_result.action {
            ReconcileAction::TransitionedToLeft => {
                self.handle_transitioned_to_left(
                    ctx.opctx, ctx.group, ctx.member,
                )
                .await
            }
    ...
    async fn complete_instance_member_join(
            ...
            sled_id_override.or(ctx.member.sled_id.map(Into::into))
        {
            ...
            self.lookup_and_update_member_sled_id(ctx.opctx, ctx.member).await?
        {
            id.into()
        } else {
            return Ok(false);
        ...
        // The member is now "Joined" in the database, so propagate includes
        // this sled in forwarding next-hops. If propagation or subscribe
        // fails below, the member remains "Joined" with incomplete sled
        // state. The reconciler's next pass converges via
