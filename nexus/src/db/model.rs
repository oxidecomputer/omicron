...
impl Ipv4Net {
    ...
    pub fn is_requestable_addr(&self, addr: Ipv4Addr) -> bool {
        self.contains(addr)
            && (
...
impl Ipv6Net {
    ...
    /// Check if an address is a valid user-requestable address for this subnet
    pub fn is_requestable_addr(&self, addr: Ipv6Addr) -> bool {
...
impl VpcSubnet {
    ...
    pub fn is_requestable_addr(
        &self,
        addr: IpAddr,
    ) -> Result<(), external::Error> {
        let subnet = match addr {
            IpAddr::V4(addr) => {
                if self.ipv4_block.is_requestable_addr(addr) {
                    return Ok(());
                }
                ipnetwork::IpNetwork::V4(self.ipv4_block.0 .0)
            }
            IpAddr::V6(addr) => {
                if self.ipv6_block.is_requestable_addr(addr) {
                    return Ok(());
                }
...
mod tests {
    ...
    #[test]
    fn test_vpc_subnet_is_requestable_addr() {
        let ipv4_block =
        ...
        // Within subnet
        assert!(subnet
            .is_requestable_addr(IpAddr::from(Ipv4Addr::new(192, 168, 1, 10)))
            .is_ok());
        // Within subnet, but reserved
        assert!(subnet
            .is_requestable_addr(IpAddr::from(Ipv4Addr::new(192, 168, 0, 1)))
            .is_err());
        // Not within subnet
        assert!(subnet
            .is_requestable_addr(IpAddr::from(Ipv4Addr::new(192, 160, 1, 1)))
            .is_err());

        // Within subnet
        assert!(subnet
            .is_requestable_addr(IpAddr::from(Ipv6Addr::new(
    ...
    #[test]
    fn test_ip_subnet_is_requestable_address() {
        let subnet = super::Ipv4Net(Ipv4Net("192.168.0.0/16".parse().unwrap()));
        assert!(subnet.is_requestable_addr("192.168.0.10".parse().unwrap()));
        assert!(subnet.is_requestable_addr("192.168.1.0".parse().unwrap()));
        assert!(!subnet.is_requestable_addr("192.168.0.0".parse().unwrap()));
        assert!(subnet.is_requestable_addr("192.168.0.255".parse().unwrap()));
        assert!(!subnet.is_requestable_addr("192.168.255.255".parse().unwrap()));

        let subnet = super::Ipv6Net(Ipv6Net("fd00::/64".parse().unwrap()));
        assert!(subnet.is_requestable_addr("fd00::a".parse().unwrap()));
        assert!(!subnet.is_requestable_addr("fd00::1".parse().unwrap()));
        assert!(subnet.is_requestable_addr("fd00::1:1".parse().unwrap()));
    }
}
