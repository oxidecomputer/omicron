...
impl DataStore {
    pub async fn volume_create(&self, volume: Volume) -> CreateResult<Volume> {
        ...
        self.pool().transaction(move |conn| {
            let maybe_volume: Option<Volume> =
                dsl::volume
            ...
            let crucible_targets = {
                let vcr: VolumeConstructionRequest = serde_json::from_str(&volume.data())
                    .map_err(|e: serde_json::Error| TxnError::CustomError(
                        VolumeCreationError::SerdeError(e)
                    ))?;

                let mut crucible_targets = CrucibleTargets::default();
                resources_associated_with_volume(&vcr, &mut crucible_targets);
                crucible_targets
            };
            ...
            for read_only_target in &crucible_targets.read_only_targets {
                diesel::update(rs_dsl::region_snapshot)
                    .filter(rs_dsl::snapshot_addr.eq(read_only_target.clone()))
                    .set(rs_dsl::volume_references.eq(rs_dsl::volume_references + 1))
                    .execute(conn)
                    .map_err(|e| TxnError::CustomError(
                        VolumeCreationError::Public(
                            public_error_from_diesel_pool(
    ...
    /// snapshots.
    pub async fn find_deleted_volume_regions(&self) -> ListResultVec<(Dataset, Region, Volume)> {
        ...
        region_dsl::region
            .inner_join(
                volume_dsl::volume
                    .on(region_dsl::volume_id.eq(volume_dsl::id))
            )
            .inner_join(
                dataset_dsl::dataset
            ...
            .left_join(
                dsl::region_snapshot
                    .on(
                        dsl::region_id.eq(region_dsl::id)
                            .and(dsl::dataset_id.eq(dataset_dsl::id))
                    )
            )
            .filter(
                dsl::volume_references.eq(0)
            ...
            .select((Dataset::as_select(), Region::as_select(), Volume::as_select()))
        ...
        self.pool().transaction(move |conn| {
            ...
            let volume = {
                ...
                if volume.time_deleted.is_none() {
                    ...
                    if let Some(resources_to_clean_up) = volume.resources_to_clean_up {
                        // return the serialized CrucibleResources
                        return Ok(serde_json::from_str(&resources_to_clean_up)
                            .map_err(|e|
                                TxnError::CustomError(
            ...
            let result = CrucibleResources {
                ...
                datasets_and_snapshots: {
                    ...
                    dsl::region_snapshot
                        ...
                        .select((Dataset::as_select(), RegionSnapshot::as_select()))
