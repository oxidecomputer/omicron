...
impl Server {
    ...
    pub async fn register_as_producer(&self) {
        ...
        nexus
            .register_as_producer(nexus.get_internal_server_address().unwrap())
            .await;
    }
}
...
impl nexus_test_interface::NexusServer for Server {
    ...
    fn get_http_server_external_address(&self) -> SocketAddr {
        self.apictx.nexus.get_external_server_address().unwrap()
    }

    fn get_http_server_internal_address(&self) -> SocketAddr {
        self.apictx.nexus.get_internal_server_address().unwrap()
    }
