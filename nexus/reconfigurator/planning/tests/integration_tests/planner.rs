...
fn create_measurement_artifacts_at_version(
    ...
    // Start with the standard set of fake artifacts.
    ArtifactSet::fake("0.0.1".parse().unwrap())
        .unwrap()
        .into_iter()
        // Filter out the measurement artifacts...
        .filter(|artifact| {
            !matches!(
                artifact.known_tags(),
                Some(KnownArtifactTags::MeasurementCorpus)
            )
        })
        // ... and add measurement artifacts with the version and hash we want.
        .chain([
            Artifact {
...
fn create_zone_artifacts_at_version(version: &ArtifactVersion) -> ArtifactSet {
    // Generate a fake set of artifacts where zones are set to `version` but the
    // rest of the artifacts remain 0.0.1, to simulate an environment that does
    // not need SP component updates.
    ArtifactSet::fake("0.0.1".parse().unwrap())
        .unwrap()
        .into_iter()
        .map(|artifact| match artifact.known_tags() {
            Some(tags @ KnownArtifactTags::Zone(_)) => Artifact {
                target_name: String::new(),
                version: version.clone(),
                tags: tags.to_tags(),
    ...
    for i in 2..=MAX_PLANNING_ITERATIONS {
        ...
        };
        {
                ...
                && summary.total_measurements_removed() == 0
            {
                ...
                // We start at 2, our measurement change puts us to 3. No other
                // changes are expected as the artifacts are otherwise the same
                assert_eq!(
...
fn test_multiple_measurements() {
    ...
    artifacts.insert(BlueprintSingleMeasurement {
        version: BlueprintArtifactVersion::Available {
            version: ArtifactVersion::new_static("0.0.1").unwrap(),
        },
        hash: ArtifactHash(hex_literal::hex!(
            "b0db8f64463a377a302082c5752056ee191adf904daa625d38a2094059f55248"
        )),
    });
    artifacts.insert(BlueprintSingleMeasurement {
        version: BlueprintArtifactVersion::Available {
        ...
        hash: ArtifactHash(hex_literal::hex!(
            "dbea4039b6ed82dbdd8d794d398a5c647ffa6991bd7d063702c2acbc7a0b90c7"
        )),
    });
    // From our new repo
    artifacts.insert(BlueprintSingleMeasurement {
