    ...
    if unsafe_zone_boards.contains_key(baseboard_id) {
        ...
        if let Some(board) = unsafe_zone_boards.get(baseboard_id) {
            for (zone_id, zone) in board {
                ...
                zone_str.push_str(&format!("* {}: {}", zone_id, zone));
            }
        };
...
mod tests {
    ...
    fn test_basic_host_os() {
        ...
        let planned = plan_mgs_updates(PlanMgsUpdatesInput {
            log,
            inventory: &collection,
            current_boards,
        ...
        // makes no changes.
        let later_planned = plan_mgs_updates(PlanMgsUpdatesInput {
            log,
            inventory: &collection,
            current_boards,
            unsafe_zone_boards: &BTreeMap::new(),
        ...
        let collection = test_boards
            ...
            .build();
        let new_planned = plan_mgs_updates(PlanMgsUpdatesInput {
            log,
            inventory: &collection,
            current_boards: &collection.baseboards,
            unsafe_zone_boards: &BTreeMap::new(),
