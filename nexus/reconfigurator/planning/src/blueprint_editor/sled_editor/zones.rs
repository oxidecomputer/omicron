...
pub struct InputDuplicateZoneId {
...
impl ZonesEditor {
    pub fn add_zone(
        ...
        match self.zones.entry(zone.id) {
            ...
            Entry::Occupied(prev) => {
                ...
                Err(EditZonesError::AddDuplicateZoneId {
                    id: zone.id,
    ...
    ) -> Result<&BlueprintZoneConfig, EditZonesError> {
        let config = self.zones.get_mut(zone_id).ok_or_else(|| {
            EditZonesError::ExpungeNonexistentZone { id: *zone_id }
        })?;
...
impl TryFrom<BlueprintZonesConfig> for ZonesEditor {
    type Error = InputDuplicateZoneId;
    ...
    fn try_from(config: BlueprintZonesConfig) -> Result<Self, Self::Error> {
        ...
        for zone in config.zones {
            match zones.entry(zone.id) {
                ...
                Entry::Occupied(prev) => {
                    return Err(InputDuplicateZoneId {
                        id: zone.id,
                        kind1: zone.zone_type.kind(),
                        kind2: prev.get().zone_type.kind(),
                    });
...
impl From<ZonesEditor> for BlueprintZonesConfig {
    fn from(editor: ZonesEditor) -> Self {
        ...
        Self { generation, zones: editor.zones.into_values().collect() }
    }
}
