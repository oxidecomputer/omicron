...
pub struct Simulator {
    ...
    // pointer to the root state to ensure that `commit` is called on it.
    // Simulator can both be cloned and moved in memory, but because
    // `root_state` is never changed, it always points to the same memory
    // address.
