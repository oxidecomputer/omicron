...
use nexus_types::{
    deployment::{
        Blueprint, BlueprintTarget, BlueprintWithPlanningReport, SledFilter,
...
pub struct SimSystem {
    ...
    ///
    /// Stored with `Arc` to allow cheap cloning.
    blueprints: IndexMap<BlueprintUuid, Arc<BlueprintWithPlanningReport>>,
...
impl SimSystem {
    ...
    ) -> Result<&BlueprintWithPlanningReport, KeyError> {
    ...
    pub fn all_blueprints(
        &self,
    ) -> impl ExactSizeIterator<Item = &BlueprintWithPlanningReport> {
...
impl SimSystemBuilder {
    ...
    pub fn add_blueprint(
        &mut self,
        blueprint: impl Into<Arc<BlueprintWithPlanningReport>>,
    ) -> Result<(), DuplicateError> {
...
impl SimSystemBuilderInner {
    ...
    fn load_example_inner(
        ...
        // The blueprints generated by the example system did not come from the
        // planner, and therefore have no meaningful planner report.
        self.add_blueprint_inner(Arc::new(
    ...
    fn load_serialized_inner(
        ...
        for blueprint in state.blueprints {
            ...
            // TODO Should we store planning reports in
            // `UnstableReconfiguratorState`? It would be Even More Unstable -
            // the reports are stored as JSON blobs in the database, and there's
            // no guarantee that all the reports in a given
            // `UnstableReconfiguratorState` would be consistent with each other
            // - they're really only intended for human / omdb consumption and
            // debugging. For now, we just construct empty reports for all
            // blueprints in the loaded state.
            match self.add_blueprint_inner(Arc::new(
                BlueprintWithPlanningReport::with_empty_report(blueprint),
            )) {
                Ok(_) => {
    ...
    fn add_blueprint_inner(
        &mut self,
        blueprint: Arc<BlueprintWithPlanningReport>,
    ) -> Result<(), DuplicateError> {
        let blueprint_id = blueprint.id;
