...
//! Nexus capabilities for updating firewall rules
...
pub trait FirewallRules: SledAgent {
    async fn vpc_list_firewall_rules(
        &self,
        opctx: &OpContext,
        ...
        let rules =
            self.datastore().vpc_list_firewall_rules(opctx, &authz_vpc).await?;
        Ok(rules)
    }

    /// Ensure firewall rules for internal services get reflected on all the
    /// relevant sleds.
    async fn plumb_service_firewall_rules(
        &self,
        opctx: &OpContext,
        sleds_filter: &[Uuid],
    ) -> Result<(), Error> {
