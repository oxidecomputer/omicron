...
// A backfilled external IP row:
// (external_ip_id, ip, port, snat_first_port, snat_last_port).
type ExternalIpRow = (Uuid, String, Option<i32>, Option<i32>, Option<i32>);
...
async fn before_impl(ctx: &MigrationContext<'_>) {
    ...
    //
    // Unlike inventory, each blueprint external IP carries its allocated
    // `external_ip_id`, so we set that column for the external-networking zones.
    let sql = format!(
...
async fn after_impl(ctx: &MigrationContext<'_>) {
    ...
    assert_eq!(
        by_zone.get(&nexus),
        Some(&(
            Uuid::parse_str(NEXUS_EIP_ID).unwrap(),
            "192.0.2.1".to_string(),
            None,
            None,
            None,
    ...
    assert_eq!(
        by_zone.get(&dns),
        Some(&(
            Uuid::parse_str(DNS_EIP_ID).unwrap(),
            "192.0.2.2".to_string(),
            Some(53),
            None,
            None,
    ...
    assert_eq!(
        by_zone.get(&ntp),
        Some(&(
            Uuid::parse_str(NTP_EIP_ID).unwrap(),
            "192.0.2.3".to_string(),
            None,
            Some(0),
            Some(16383),
