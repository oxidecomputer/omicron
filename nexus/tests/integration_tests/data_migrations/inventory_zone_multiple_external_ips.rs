...
//! Data migration test for moving inventory zone external IPs from inline
//! columns on `inv_omicron_sled_config_zone` into the new
//! `inv_omicron_sled_config_zone_external_ip` child table.
...
// A backfilled external IP row: (ip, port, snat_first_port, snat_last_port).
type ExternalIpRow = (String, Option<i32>, Option<i32>, Option<i32>);

// IDs for the inserted rows.
const COLLECTION_ID: &str = "d29f7b65-9b2b-4650-9b80-9ba968468083";
const CONFIG_ID: &str = "1fda886f-b3e4-4068-9e20-b29d7d4c9190";
...
async fn before_impl(ctx: &MigrationContext<'_>) {
    // Insert inventory zone rows with the old, inlined external IPs directly
    // in the row. The zones are:
    //
    ...
    let sql = format!(
        "INSERT INTO omicron.public.inv_omicron_sled_config_zone (
            inv_collection_id, sled_config_id, id, zone_type,
            primary_service_ip, primary_service_port,
            second_service_ip, second_service_port,
            snat_ip, snat_first_port, snat_last_port,
            nexus_lockstep_port, image_source
        ) VALUES
        ('{COLLECTION_ID}', '{CONFIG_ID}', '{NEXUS_ZONE_ID}', 'nexus',
         '::1', 12345, '192.0.2.1', NULL, NULL, NULL, NULL, 12346,
         'install_dataset'),
        ('{COLLECTION_ID}', '{CONFIG_ID}', '{DNS_ZONE_ID}', 'external_dns',
         '::1', 5353, '192.0.2.2', 53, NULL, NULL, NULL, NULL,
         'install_dataset'),
        ('{COLLECTION_ID}', '{CONFIG_ID}', '{NTP_ZONE_ID}', 'boundary_ntp',
         '::1', 123, NULL, NULL, '192.0.2.3', 0, 16383, NULL,
         'install_dataset'),
        ('{COLLECTION_ID}', '{CONFIG_ID}', '{INTERNAL_DNS_ZONE_ID}', \
         'internal_dns',
         '::1', 5353, '{INTERNAL_DNS_IP}', {INTERNAL_DNS_PORT}, NULL, NULL, \
         NULL, NULL, 'install_dataset'),
        ('{COLLECTION_ID}', '{CONFIG_ID}', '{CRUCIBLE_ZONE_ID}', 'crucible',
         '::1', 32345, NULL, NULL, NULL, NULL, NULL, NULL,
         'install_dataset')",
    );
    ctx.client
        .batch_execute(&sql)
        .await
        .expect("inserted old-schema inventory zone rows");
}
...
async fn after_impl(ctx: &MigrationContext<'_>) {
    let collection_id = Uuid::parse_str(COLLECTION_ID).unwrap();
    let rows = ctx
        .client
        .query(
            "SELECT \
            ...
            WHERE inv_collection_id = $1",
            &[&collection_id],
        )
        .await
        .expect("queried external IP child table");

    let mut by_zone: BTreeMap<Uuid, ExternalIpRow> = BTreeMap::new();
    for row in rows {
        ...
        by_zone.insert(zone_id, (ip, port, snat_first_port, snat_last_port));
    }
    ...
    let nexus = Uuid::parse_str(NEXUS_ZONE_ID).unwrap();
    assert_eq!(
        by_zone.get(&nexus),
        Some(&("192.0.2.1".to_string(), None, None, None)),
        "Nexus external IP should be a floating IP with no ports",
    );

    let dns = Uuid::parse_str(DNS_ZONE_ID).unwrap();
    assert_eq!(
        by_zone.get(&dns),
        Some(&("192.0.2.2".to_string(), Some(53), None, None)),
        "external DNS external IP should carry its DNS port",
    );

    let ntp = Uuid::parse_str(NTP_ZONE_ID).unwrap();
    assert_eq!(
        by_zone.get(&ntp),
        Some(&("192.0.2.3".to_string(), None, Some(0), Some(16383))),
        "boundary NTP external IP should carry its SNAT port range",
    );

    // Crucible has only the primary IP and no external IPs.
    let crucible = Uuid::parse_str(CRUCIBLE_ZONE_ID).unwrap();
    assert!(
    ...
    let zone_rows = ctx
        ...
        .await
        .expect("queried inventory zone rows");

    let mut second_service_by_zone: BTreeMap<
        Uuid,
    ...
    for row in zone_rows {
        ...
        second_service_by_zone.insert(id, (ip, port));
    }
