...
pub const DEMO_CONTROL_PLANE_ROUTER_CONFIGURATIONS_URL: &'static str =
    "/v1/system/networking/control-plane-router-configurations";
pub static DEMO_CONTROL_PLANE_ROUTER_CONFIGURATIONS_UPDATE: LazyLock<
    networking::ControlPlaneRouterConfigurationsUpdate,
> = LazyLock::new(|| networking::ControlPlaneRouterConfigurationsUpdate {
    configurations: vec![networking::ControlPlaneRouterConfigurationEntry {
        router_configuration: NameOrId::Name(
            "demo-router-configuration".parse().unwrap(),
        ),
        priority: 0,
    }],
});
    ...
    || {
        vec![
            ...
            */
            ...
            VerifyEndpoint {
                url: &DEMO_CONTROL_PLANE_ROUTER_CONFIGURATIONS_URL,
                visibility: Visibility::Public,
                unprivileged_access: UnprivilegedAccess::None,
                allowed_methods: vec![
                    AllowedMethod::Get,
                    AllowedMethod::Put(
                        serde_json::to_value(
                            &*DEMO_CONTROL_PLANE_ROUTER_CONFIGURATIONS_UPDATE,
                        )
                        .unwrap(),
                    ),
                ],
