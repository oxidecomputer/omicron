...
async fn test_create_a_saml_idp(cptestctx: &ControlPlaneTestContext) {
    ...
    let silo_saml_idp: SiloSamlIdentityProvider = object_create(
        client,
        &format!("/silos/{}/saml_identity_provider", SILO_NAME),
    ...
    let _retrieved_silo_saml_idp: SiloSamlIdentityProvider =
        NexusRequest::object_get(
            &client,
            &format!(
                "/silos/{}/saml_identity_provider/{}",
...
// TODO when external-authenticator works with multiple silos
/*
#[nexus_test]
async fn test_deleting_a_silo_deletes_the_idp(
    ...
    let _silo_saml_idp: SiloSamlIdentityProvider = object_create(
        client,
        &format!("/silos/{}/saml_identity_provider", SILO_NAME),
        &params::SiloSamlIdentityProviderCreate {
            identity: IdentityMetadataCreateParams {
