...
impl DataMigrationFns {
    ...
    pub(crate) fn before(mut self, before: BeforeFn) -> Self {
        self.before = Some(before);
        self
    }
    pub(crate) fn after(mut self, after: AfterFn) -> Self {
...
async fn validate_data_migration_from_version_to_target(
    ...
    let all_versions = read_all_schema_versions();
    let mut all_checks = super::data_migrations::get_migration_checks();
...
// - Create a new file in data_migrations/ named after your migration directory
//   (with hyphens replaced by underscores), e.g., `my_migration.rs`.
// - Define `pub(crate) fn before` and/or `pub(crate) fn after` in that file.
// - Add a `mod` declaration and `map.insert` entry in `data_migrations/mod.rs`,
//   using `version_for("my-migration")` to look up the version number.
