...
mod migration_243 {
    ...
    // SP ereports at location (sp, sled, slot 0), with three restart IDs
    const SLED0_SP_RESTART0: Uuid =
        Uuid::from_u128(24300001_0000_0000_0000_000000000003);
    const SLED0_SP_RESTART1: Uuid =
        Uuid::from_u128(24300001_0000_0000_0000_000000000001);
    const SLED0_SP_RESTART2: Uuid =
        Uuid::from_u128(24300001_0000_0000_0000_000000000002);

    // SP ereports at a different location (sp, switch, slot 1)
    const SWITCH1_SP_RESTART1: Uuid =
        Uuid::from_u128(24300002_0000_0000_0000_000000000001);

    // Host OS ereports at the *same* location as the SP ereports above
    // (host, sled, slot 0). These should get an independent generation
    // sequence from the SP restarts, since the reporter type differs.
    const SLED0_HOST_RESTART0: Uuid =
        Uuid::from_u128(24300007_0000_0000_0000_000000000001);
    const SLED0_HOST_RESTART1: Uuid =
        Uuid::from_u128(24300007_0000_0000_0000_000000000002);

    // Host OS ereports at location (host, sled, slot 7)
    const SLED7_HOST_RESTART0: Uuid =
        Uuid::from_u128(24300003_0000_0000_0000_000000000002);
    const SLED7_HOST_RESTART1: Uuid =
        Uuid::from_u128(24300003_0000_0000_0000_000000000001);

    // Host OS ereport with NULL slot (should be excluded from backfill)
    const HOST_NO_SLOT: Uuid =
        Uuid::from_u128(24300004_0000_0000_0000_000000000001);

    // Host OS restart where the first ereport has a NULL slot (the
    // sled's physical location is not yet known), and a subsequent
    // ereport with the same restart ID has slot = 5 (location
    // discovered later). This should still be backfilled, using only
    // the non-NULL-slot ereports.
    const SLED5_HOST_RESTART0: Uuid =
        Uuid::from_u128(24300008_0000_0000_0000_000000000001);

    const SLED0_SLED_ID: Uuid =
        Uuid::from_u128(24300005_0000_0000_0000_000000000003);
    const SLED7_SLED_ID: Uuid =
        Uuid::from_u128(24300005_0000_0000_0000_000000000001);
    const NO_SLOT_SLED_ID: Uuid =
        Uuid::from_u128(24300005_0000_0000_0000_000000000002);
    const SLED5_SLED_ID: Uuid =
        Uuid::from_u128(24300005_0000_0000_0000_000000000004);

    const COLLECTOR: Uuid =
        Uuid::from_u128(24300006_0000_0000_0000_000000000001);
        ...
        Box::pin(async move {
            ctx.client
                .batch_execute(&format!(
                    "INSERT INTO omicron.public.ereport (
                        restart_id, ena, time_collected, collector_id,
                        report, reporter, sled_id, slot_type, slot
                    ) VALUES
                        -- SP restarts at (sp, sled, slot 0).
                        -- OLDEST has two ereports so we can verify
                        -- MIN(time_collected) is used for time_first_seen.
                        ('{SLED0_SP_RESTART0}', 1,
                         '2025-01-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'sp', NULL, 'sled', 0),
                        ('{SLED0_SP_RESTART0}', 2,
                         '2025-12-15 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'sp', NULL, 'sled', 0),
                        -- middle restart
                        ('{SLED0_SP_RESTART1}', 1,
                         '2025-06-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'sp', NULL, 'sled', 0),
                        -- newest restart
                        ('{SLED0_SP_RESTART2}', 1,
                         '2025-12-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'sp', NULL, 'sled', 0),

                        -- SP restart at different location (sp, switch, 1)
                        ('{SWITCH1_SP_RESTART1}', 1,
                         '2025-03-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'sp', NULL, 'switch', 1),

                        -- Host restarts at (host, sled, slot 7)
                        ('{SLED7_HOST_RESTART0}', 1,
                         '2025-02-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'host', '{SLED7_SLED_ID}',
                         'sled', 7),
                        ('{SLED7_HOST_RESTART1}', 1,
                         '2025-08-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'host', '{SLED7_SLED_ID}',
                         'sled', 7),

                        -- Host restarts at (host, sled, slot 0) ---
                        -- same slot as the SP ereports, but reporter
                        -- type is 'host' so generations are independent.
                        ('{SLED0_HOST_RESTART0}', 1,
                         '2025-05-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'host', '{SLED0_SLED_ID}',
                         'sled', 0),
                        ('{SLED0_HOST_RESTART1}', 1,
                         '2025-09-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'host', '{SLED0_SLED_ID}',
                         'sled', 0),

                        -- Host ereport with NULL slot (excluded from backfill)
                        ('{HOST_NO_SLOT}', 1,
                         '2025-04-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'host', '{NO_SLOT_SLED_ID}',
                         'sled', NULL),

                        -- Host restart where the first ereport has a
                        -- NULL slot (location not yet known), and a
                        -- later ereport has slot = 5 (location
                        -- discovered).  The restart should still be
                        -- backfilled using the non-NULL-slot ereport.
                        ('{SLED5_HOST_RESTART0}', 1,
                         '2025-03-15 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'host', '{SLED5_SLED_ID}',
                         'sled', NULL),
                        ('{SLED5_HOST_RESTART0}', 2,
                         '2025-07-01 00:00:00+00', '{COLLECTOR}',
                         '{{}}', 'host', '{SLED5_SLED_ID}',
                         'sled', 5);
                    "
                ))
    ...
    pub(super) fn after<'a>(
        ...
        Box::pin(async move {
            // Query all restart rows that the backfill may have created.
            let rows = ctx
                .client
                .query(
                    &format!(
                ...
                .await
                .expect("failed to query ereporter_restart rows");

            let results: HashMap<Uuid, (i64, String, String, i32)> = rows
                .iter()
                .map(|row| {
                    let id: Uuid = row.get("id");
                    let generation: i64 = row.get("generation");
                    let reporter_type: String = row.get("reporter_type");
                    let slot_type: String = row.get("slot_type");
                    let slot: i32 = row.get("slot");
                    (id, (generation, reporter_type, slot_type, slot))
                })
                .collect();

            // The host ereport with NULL slot must NOT have been backfilled.
            assert!(
                !results.contains_key(&HOST_NO_SLOT),
                "ereport with NULL slot should not appear in \
                 ereporter_restart"
            );

            // We expect exactly 9 rows:
            // - 3 restarts of (sp, sled, 0)
            // - 1 restart of (sp, switch, 1)
            // - 2 restarts of (host, sled, 0)
            // - 2 restarts of (host, sled, 7)
            // - 1 restart of (host, sled, 5) --- initially NULL slot, later discovered
            assert_eq!(
                results.len(),
                9,
                "expected 9 backfilled ereporter_restart rows, got {}: \
                 {results:#?}",
                results.len()
            );

            // restart IDs for (sp, sled, slot 0):
            // (2025-01-01) -> generation 0
            // (2025-06-01) -> generation 1
            // (2025-12-01) -> generation 2
            let (generation, rtype, stype, s) = &results[&SLED0_SP_RESTART0];
            assert_eq!(
                *generation, 0,
                "oldest sled 0 SP restart should be generation 0"
            );
            assert_eq!(rtype, "sp");
            assert_eq!(stype, "sled");
            assert_eq!(*s, 0);

            let (generation, rtype, stype, s) = &results[&SLED0_SP_RESTART1];
            assert_eq!(
                *generation, 1,
                "middle sled 0 SP restart should be generation 1"
            );
            assert_eq!(rtype, "sp");
            assert_eq!(stype, "sled");
            assert_eq!(*s, 0);

            let (generation, rtype, stype, s) = &results[&SLED0_SP_RESTART2];
            assert_eq!(
                *generation, 2,
                "newest sled 0 SP restart should be generation 2"
            );
            assert_eq!(rtype, "sp");
            assert_eq!(stype, "sled");
            assert_eq!(*s, 0);

            // only 1 restart ID for (sp, switch, slot 1)
            let (generation, rtype, stype, s) = &results[&SWITCH1_SP_RESTART1];
            assert_eq!(
                *generation, 0,
                "switch SP restart should be generation 0"
            );
            assert_eq!(rtype, "sp");
            assert_eq!(stype, "switch");
            assert_eq!(*s, 1);

            // restart IDs for (host, sled, slot 7):
            // (2025-02-01) -> generation 0
            // (2025-08-01) -> generation 1
            let (generation, rtype, stype, s) = &results[&SLED7_HOST_RESTART0];
            assert_eq!(
                *generation, 0,
                "older sled 7 host restart should be generation 0"
            );
            assert_eq!(rtype, "host");
            assert_eq!(stype, "sled");
            assert_eq!(*s, 7);

            let (generation, rtype, stype, s) = &results[&SLED7_HOST_RESTART1];
            assert_eq!(
                *generation, 1,
                "newer sled 7 host restart should be generation 1"
            );
            assert_eq!(rtype, "host");
            assert_eq!(stype, "sled");
            assert_eq!(*s, 7);

            // restart IDs for (host, sled, slot 0):
            // Same physical slot as the SP ereports above, but with
            // reporter_type = 'host' instead of 'sp', so the generation
            // sequence is independent.
            // 2025-05-01 -> generation 0
            // 2025-09-01 -> generation 1
            let (generation, rtype, stype, s) = &results[&SLED0_HOST_RESTART0];
            assert_eq!(
                *generation, 0,
                "older sled host OS restart should be generation 0 \
                 (independent from SP restarts at the same slot)"
            );
            assert_eq!(rtype, "host");
            assert_eq!(stype, "sled");
            assert_eq!(*s, 0);

            let (generation, rtype, stype, s) = &results[&SLED0_HOST_RESTART1];
            assert_eq!(
                *generation, 1,
                "newer sled 0 host OS restart should be generation 1 \
                 (independent from SP restarts at the same slot)"
            );
            assert_eq!(rtype, "host");
            assert_eq!(stype, "sled");
            ...
            // restart ID for (host, sled, slot 5):
            // This restart had an initial ereport with NULL slot
            // (location unknown) followed by one with slot = 5
            // (location discovered).  It should be backfilled using
            // the non-NULL-slot ereport.
            let (generation, rtype, stype, s) = &results[&SLED5_HOST_RESTART0];
            assert_eq!(
                *generation, 0,
                "sled 5 host restart with late slot discovery should be \
                 generation 0 (only restart at this location)"
            );
            assert_eq!(rtype, "host");
            assert_eq!(stype, "sled");
            assert_eq!(*s, 5);
            ...
            ctx.client
                ...
                .await
                .expect("failed to clean up migration 243 test data");
        })
    }
}
...
fn get_migration_checks() -> BTreeMap<Version, DataMigrationFns> {
    ...
    map.insert(
        Version::new(244, 0, 0),
        DataMigrationFns::new()
            .before(migration_244::before)
            .after(migration_244::after),
    );
    map
}
