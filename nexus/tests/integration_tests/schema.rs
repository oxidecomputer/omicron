...
fn before_205_0_0<'a>(ctx: &'a MigrationContext<'a>) -> BoxFuture<'a, ()> {
    Box::pin(async move {
        ctx.client
            .execute(
...
fn after_205_0_0<'a>(ctx: &'a MigrationContext<'a>) -> BoxFuture<'a, ()> {
    Box::pin(async move {
        // first disk
...
fn get_migration_checks() -> BTreeMap<Version, DataMigrationFns> {
    ...
    map.insert(
        Version::new(205, 0, 0),
        DataMigrationFns::new().before(before_205_0_0).after(after_205_0_0),
    );
    map
}
