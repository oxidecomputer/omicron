...
async fn test_router_configuration_basic_crud(ctx: &ControlPlaneTestContext) {
    ...
    // Initially there are only the built-in per-switch configurations.
    ...
    assert_eq!(configurations[0].identity.name, "default-switch0");
    assert_eq!(configurations[0].switch, SwitchSlot::Switch0);
    assert_eq!(configurations[1].identity.name, "default-switch1");
    assert_eq!(configurations[1].switch, SwitchSlot::Switch1);
    ...
    assert!(
        configurations.iter().any(|c| c.identity.id == created.identity.id)
    );
...
#[nexus_test]
async fn test_builtin_router_configurations(ctx: &ControlPlaneTestContext) {
    ...
    // Both built-ins exist with their fixed ids and switches.
    for (name, id, switch) in [
        (
            "default-switch0",
            DEFAULT_SWITCH0_ROUTER_CONFIGURATION_ID,
            SwitchSlot::Switch0,
        ...
        let url = format!("{CONFIGURATIONS_URL}/{name}");
        let fetched: RouterConfiguration =
            NexusRequest::object_get(client, &url)
                ...
                .parsed_body()
                .unwrap();
        assert_eq!(fetched.identity.id, id);
        assert_eq!(fetched.switch, switch);
    }

    // Reserved names cannot be minted.
    for name in ["default", "default-switch0", "default-switch1"] {
        let params = RouterConfigurationCreate {
            identity: IdentityMetadataCreateParams {
    ...
    let builtin_url = format!("{CONFIGURATIONS_URL}/default-switch0");

    // A built-in cannot be renamed, ...
    let rename = RouterConfigurationUpdate {
        identity: IdentityMetadataUpdateParams {
            name: Some("liberated".parse().unwrap()),
            description: None,
        },
        switch: None,
    };
    NexusRequest::new(
        RequestBuilder::new(client, Method::PUT, &builtin_url)
            .body(Some(&rename))
            .expect_status(Some(StatusCode::BAD_REQUEST)),
    )
    .authn_as(AuthnMode::PrivilegedUser)
    ...
    // ... cannot change switch, ...
    let move_switch = RouterConfigurationUpdate {
        identity: IdentityMetadataUpdateParams {
            name: None,
            description: None,
        },
    ...
    // ... but a same-name/same-switch (description-only) update is fine.
    let describe = RouterConfigurationUpdate {
        identity: IdentityMetadataUpdateParams {
            name: Some("default-switch0".parse().unwrap()),
            description: Some("still the built-in".into()),
        },
        switch: Some(SwitchSlot::Switch0),
    };
    let updated: RouterConfiguration =
        NexusRequest::object_put(client, &builtin_url, Some(&describe))
            .authn_as(AuthnMode::PrivilegedUser)
            .execute()
            .await
            .unwrap()
            .parsed_body()
            .unwrap();
    assert_eq!(updated.identity.description, "still the built-in");

    // A normal configuration cannot take a reserved name.
    let params = RouterConfigurationCreate {
        identity: IdentityMetadataCreateParams {
            name: "routy".parse().unwrap(),
            description: "just a router configuration".into(),
    ...
    NexusRequest::objects_post(client, CONFIGURATIONS_URL, &params)
        ...
        .await
        .unwrap();
    let routy_url = format!("{CONFIGURATIONS_URL}/routy");
    for name in ["default", "default-switch1"] {
        let steal = RouterConfigurationUpdate {
            identity: IdentityMetadataUpdateParams {
        ...
        NexusRequest::new(
            RequestBuilder::new(client, Method::PUT, &routy_url)
                .body(Some(&steal))
                .expect_status(Some(StatusCode::BAD_REQUEST)),
        )
        .authn_as(AuthnMode::PrivilegedUser)
