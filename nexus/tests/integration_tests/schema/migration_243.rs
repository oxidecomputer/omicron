...
//! Data migration test for BGP unnumbered peer cleanup (migration 243).
//!
...
const PORT_SETTINGS: Uuid =
    Uuid::from_u128(0x24300001_0000_0000_0000_000000000001);
const BGP_CONFIG: Uuid =
    Uuid::from_u128(0x24300001_0000_0000_0000_000000000002);

// bgp_peer_config row IDs
const PEER_NUMBERED: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000001);
const PEER_SINGLE_NULL: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000002);
const PEER_SINGLE_V4: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000003);
const PEER_SINGLE_V6: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000004);
const PEER_NULL_V4_NULL: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000005);
const PEER_NULL_V4_V4: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000006);
const PEER_NULL_V6_NULL: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000007);
const PEER_NULL_V6_V6: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000008);
const PEER_NULL_BOTH_NULL: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_000000000009);
const PEER_NULL_BOTH_V4: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_00000000000a);
const PEER_NULL_BOTH_V6: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_00000000000b);
const PEER_V4_V6_V4: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_00000000000c);
const PEER_V4_V6_V6: Uuid =
    Uuid::from_u128(0x24300002_0000_0000_0000_00000000000d);

pub(super) fn before<'a>(ctx: &'a MigrationContext<'a>) -> BoxFuture<'a, ()> {
    Box::pin(async move {
        ctx.client
            ...
            .await
            .expect("failed to insert test data for migration 243");
    })
}

pub(super) fn after<'a>(ctx: &'a MigrationContext<'a>) -> BoxFuture<'a, ()> {
    Box::pin(async move {
        ...
        ctx.client
            ...
            .await
            .expect("cleanup migration 243 test data");
    })
}
