...
#[nexus_test]
async fn test_silo_ip_pool_basic_crud(cptestctx: &ControlPlaneTestContext) {
    test_ip_pool_basic_crud_impl(cptestctx, IpPoolAssignment::Silos).await;
}

#[nexus_test]
async fn test_system_ip_pool_basic_crud(cptestctx: &ControlPlaneTestContext) {
    test_ip_pool_basic_crud_impl(cptestctx, IpPoolAssignment::SystemServices)
        .await;
}

async fn test_ip_pool_basic_crud_impl(
    ...
    let list = get_ip_pools(client, Some(assignment)).await;
    assert_eq!(list.len(), count + 1, "Expected exactly 1 new IP pool");
    let new_pool = list
        .iter()
        .find(|p| !original_ip_pools.iter().any(|op| op.id() == p.id()))
        .expect("Should have listed the new pool we just made");
    assert_pools_eq(&created_pool, new_pool);
...
async fn get_ip_pools(
    client: &ClientTestContext,
    assignment: Option<IpPoolAssignment>,
) -> Vec<IpPool> {
    let items = NexusRequest::iter_collection_authn::<IpPool>(
        client,
        "/v1/system/ip-pools",
        "",
        None,
    ...
    if let Some(assignment) = assignment {
        items.into_iter().filter(|p| p.assignment == assignment).collect()
    } else {
        items
    }
}
...
async fn test_ip_pool_pagination(cptestctx: &ControlPlaneTestContext) {
    ...
    // Filter to the IP Pools assigned to silos.
    let first_five_url = format!("{}?limit=5&assignment=silos", base_url);
    let first_five =
    ...
    assert!(
        first_five
            .items
            .iter()
            .all(|p| p.assignment == IpPoolAssignment::Silos)
    );
    ...
    // Test pagination filtering to the one system IPv6 pool we expect.
    let url = format!("{}?assignment=system_services&ip_version=v6", base_url);
    let pools = objects_list_page_authz::<IpPool>(client, &url).await;
    assert_eq!(pools.items.len(), 1);
    assert_eq!(pools.items[0].ip_version, IpVersion::V6);
    assert_eq!(pools.items[0].assignment, IpPoolAssignment::SystemServices);
}
