...
use nexus_test_utils::resource_helpers::{
    ...
    objects_list_page_authz, test_params,
};
...
async fn test_audit_log_list(ctx: &ControlPlaneTestContext) {
    ...
    // we have to do this rigmarole for this request in order to get the user agent header in there
    ...
    assert_eq!(e2.user_agent.as_ref().unwrap(), "A pretend user agent string");
