...
async fn test_unauthorized(cptestctx: &ControlPlaneTestContext) {
    ...
    // Create test data with hardcoded values.
    ...
    // Create test data with non-hardcoded values.

    // Create a GlobalImage
    let demo_image_create: params::ImageCreate = params::ImageCreate {
    ...
    NexusRequest::objects_post(client, "/images", &demo_image_create)
        .authn_as(AuthnMode::PrivilegedUser)
        .execute()
        .await
        .unwrap();
    ...
    let demo_silo_saml_idp_create = params::SiloSamlIdentityProviderCreate {
        identity: IdentityMetadataCreateParams {
            name: "demo-saml-provider".to_string().parse().unwrap(),
            description: "a demo provider".to_string(),
        },

        silo_id: DEFAULT_SILO.id().clone(),
    ...
    NexusRequest::objects_post(
        client,
        "/silos/default-silo/saml_identity_provider",
        &demo_silo_saml_idp_create,
    )
    .authn_as(AuthnMode::PrivilegedUser)
    .execute()
    .await
    ...
    let mut non_hardcoded_endpoints = vec![
        /* Project images */
        VerifyEndpoint {
        ...
        VerifyEndpoint {
            ...
            visibility: Visibility::Protected,
            allowed_methods: vec![AllowedMethod::Get, AllowedMethod::Delete],
        },
        ...
        VerifyEndpoint {
            ...
            allowed_methods: vec![AllowedMethod::Post(
                serde_json::to_value(&demo_silo_saml_idp_create).unwrap(),
            )],
        },
        VerifyEndpoint {
            url:
    ...
    let mut endpoints: Vec<VerifyEndpoint> = Vec::with_capacity(
        VERIFY_ENDPOINTS.len() + non_hardcoded_endpoints.len(),
    );
    endpoints.extend(VERIFY_ENDPOINTS.clone());
    endpoints.append(&mut non_hardcoded_endpoints);
    ...
    // Validate unauthorized coverage
    unauthorized_coverage(&endpoints);
}
...
fn unauthorized_coverage(endpoints: &Vec<VerifyEndpoint>) {
