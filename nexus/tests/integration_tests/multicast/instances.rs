...
async fn test_multicast_lifecycle(cptestctx: &ControlPlaneTestContext) {
    ...
    // Verify both instances are attached to group-lifecycle-2 ("Left"
...
/// sled-agent multicast state convergence that sequential tests can't catch.
#[nexus_test]
...
///    follows the instance placement
/// 2. Concurrent migrations: multiple instances migrate simultaneously without interference
///
/// The RPW reconciler detects `sled_id` changes, updates member placement, and
/// re-converges OPTE subscription plus M2P/forwarding state. Per-member
/// rear-port DPD programming is owned by `ddmd` from DDM peer subscriptions.
...
async fn test_multicast_migration_scenarios(
    ...
    // not have any subscription for the old propolis.
    {
        ...
        // Target sled should have the VMM subscription. The target VMM
        // receives memberships at registration, applied when OPTE ports
        // are created. The reconciler's drift path is a backstop when the
        // instance-update saga's eager sled_id write fails. Poll because
        // registration and reconciliation may still be settling.
        ...
        let propolis1_id = cptestctx
            ...
            .await
            .unwrap()
            .expect("instance1 should have an active VMM")
            .propolis_id;
...
/// in the same group.
///
/// This test forces a multi-sled layout via migration (A and B share a sled,
/// C sits on the other) so the member rows Nexus tracks span both shared and
/// solo placement when B is deleted. Per-member rear-port forwarding in DPD
/// is owned by `ddmd`, derived from DDM peer subscriptions, and is not asserted
/// here. The
/// group-level invariants are that the group stays "Active", its underlay
/// group survives in DPD on every switch, and its MRIB route persists.
#[nexus_test(extra_sled_agents = 1)]
async fn test_instance_delete_preserves_other_memberships(
    ...
    // Group-level DPD state is all Nexus owns. Per-member rear-port
    // forwarding is `ddmd`'s job from DDM peer subscriptions and is not
    // asserted here. Assert only that the underlay group exists on every
    // switch.
    for (slot, dpd) in nexus_test_utils::dpd_clients_by_switch(cptestctx) {
        dpd.multicast_group_get_underlay(&underlay_admin_ip)
            .await
            .unwrap_or_else(|e| {
                panic!(
                    "{slot:?}: underlay group should exist in DPD before \
                     B's deletion: {e}"
                )
            });
    }

    // The ddm-peers view backs `omdb nexus multicast ddm-peers`. No rear-port
    // peers form under this harness, so assert only that the read path is
    // reachable and well-formed.
    assert_ddm_peers_view_reachable(cptestctx).await;

    // Delete instance B. A still occupies the shared sled and C its own,
    // so the group must stay "Active" with its dataplane state intact.
    ...
    // B's deletion must not disturb the group's dataplane state: the
    // underlay group survives on every switch (A holds the shared sled) and
    // the MRIB route persists.
    for (slot, dpd) in nexus_test_utils::dpd_clients_by_switch(cptestctx) {
        dpd.multicast_group_get_underlay(&underlay_admin_ip)
            .await
            .unwrap_or_else(|e| {
                panic!(
                    "{slot:?}: underlay group should survive B's deletion: \
                     {e}"
                )
            });
    }
    ...
    // Cleanup: deleting A and C empties the group, so the sweep tears it
    // down everywhere.
    cleanup_instances(
        cptestctx,
        client,
...
async fn test_group_with_all_members_left(cptestctx: &ControlPlaneTestContext) {
    ...
    // With no member "Joined", the reconciler withdraws the MRIB routes
    // even though the group itself stays "Active".
    assert_mrib_route_absent(cptestctx, group.multicast_ip).await;
    ...
    // The rejoin restores the any-source wildcard route.
    assert_mrib_route_sources(
        cptestctx,
        group.multicast_ip,
        &[None],
        None,
        "wildcard MRIB route restored after a member rejoins",
    )
    .await;
...
/// Test that the reconciler sweep reaps an emptied "Active" group on its own.
///
/// The synchronous mark in the detach path is a best-effort latency
/// optimization, and `cleanup_empty_groups` makes the authoritative emptiness
/// decision. This
/// test soft-deletes the last member row directly in the datastore, bypassing
/// the detach API and its fast-path mark, then verifies a reconciler sweep
/// alone marks the group "Deleting" and removes the dataplane state from DPD.
#[nexus_test]
async fn test_empty_active_group_reaped_by_sweep(
    cptestctx: &ControlPlaneTestContext,
) {
    ensure_multicast_test_ready(cptestctx).await;

    let client = &cptestctx.external_client;
    let project_name = "sweep-reap-project";
    let group_name = "sweep-reap-group";
    let instance_name = "sweep-reap-instance";

    ops::join3(
        create_default_ip_pools(client),
        create_project(client, project_name),
        create_multicast_ip_pool(client, "sweep-reap-pool"),
    )
    .await;
    ...
    instance_simulate(nexus, &instance_id).await;
    instance_wait_for_state(client, instance_id, InstanceState::Running).await;
    wait_for_all_members_joined(cptestctx, group_name).await;

    // Capture the underlay multicast IP before deletion so DPD removal can
    // be asserted after the group row is gone.
    let datastore = nexus.datastore();
    let opctx =
        OpContext::for_tests(cptestctx.logctx.log.clone(), datastore.clone());
    let group_view = get_multicast_group(client, group_name).await;
    ...
    let underlay_group = datastore
        ...
        .expect("should fetch underlay multicast group");
    let underlay_multicast_ip = underlay_group.multicast_ip.ip();
    let underlay_v6 = match underlay_multicast_ip {
        IpAddr::V6(v6) => v6,
        IpAddr::V4(_) => panic!("underlay multicast IP must be IPv6"),
    };

    // While a group is "Active", Nexus programs the handoff to mg-lower: a
    // wildcard-source MRIB route targeting the admin-local underlay group on
    // every switch's mgd.
    assert_mrib_route_sources(
        cptestctx,
        group_view.multicast_ip,
        &[None],
        Some(underlay_v6),
        "wildcard MRIB route while group is \"Active\"",
    )
    .await;
    ...
    // EXISTS guard in `mark_multicast_group_for_removal_if_no_members`
    // ignores soft-deleted rows, so no hard-delete pass is required first.
    let group_id =
    ...
    datastore
        ...
        .await
        .expect("should soft-delete member");

    // One reconciler pass runs `cleanup_empty_groups` (marks "Deleting")
    // before `reconcile_deleting_groups` (tears down dataplane state), so
    // the assertions below need no extra orchestration: group gone from
    // the API, MRIB route withdrawn from every mgd, and the underlay group
    // removed from DPD.
    wait_for_group_deleted(cptestctx, group_name).await;
    assert_mrib_route_absent(cptestctx, group_view.multicast_ip).await;
    wait_for_group_deleted_from_dpd(cptestctx, underlay_multicast_ip).await;

    // The instance itself is unaffected by the group reap.
    cleanup_instances(cptestctx, client, project_name, &[instance_name]).await;
}
