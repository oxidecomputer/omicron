...
use nexus_test_utils::resource_helpers::{create_default_ip_pools, create_project};
...
async fn test_ip_version_disambiguation(cptestctx: &ControlPlaneTestContext) {
    ...
    assert!(
        error.message.contains("ambiguous")
            || error.message.contains("ip_version")
            || error.message.contains("IPv4")
            || error.message.contains("IPv6"),
        "Expected error about ambiguous IP version, got: {}",
        error.message
    );
