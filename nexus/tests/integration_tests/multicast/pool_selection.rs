...
/// Test pool exhaustion error when all addresses in the pool are allocated.
///
/// When all multicast IPs in a pool are used, joining a new group by name
/// should return an appropriate error.
#[nexus_test]
async fn test_pool_exhaustion(cptestctx: &ControlPlaneTestContext) {
    let client = &cptestctx.external_client;

    // Setup: create a tiny pool with only 2 addresses
    ops::join3(
        create_default_ip_pools(client),
        create_project(client, PROJECT_NAME),
        create_multicast_ip_pool_with_range(
            ...
            "tiny-pool",
            (224, 1, 0, 1),
            (224, 1, 0, 2), // Only 2 IPs available
        ),
    )
    .await;
    ensure_multicast_test_ready(cptestctx).await;
    ...
    // Join first group - should succeed
    let join_url1 = format!(
        "/v1/instances/{}/multicast-groups/group-1?project={PROJECT_NAME}",
        instance.identity.id
    );
    put_upsert::<_, MulticastGroupMember>(
        client,
    ...
    // Join second group - should succeed (uses last IP)
    let join_url2 = format!(
        "/v1/instances/{}/multicast-groups/group-2?project={PROJECT_NAME}",
        instance.identity.id
    );
    put_upsert::<_, MulticastGroupMember>(
        client,
    ...
    // Join third group - should fail (pool exhausted)
    let join_url3 = format!(
        "/v1/instances/{}/multicast-groups/group-3?project={PROJECT_NAME}",
        instance.identity.id
    );

    // Pool exhaustion returns 507 Insufficient Storage
    let error: dropshot::HttpErrorResponseBody = NexusRequest::new(
        RequestBuilder::new(client, http::Method::PUT, &join_url3)
    ...
    assert_eq!(
        ...
        "Expected InsufficientCapacity error for pool exhaustion, got: {:?}",
        error.error_code
    );
}
...
/// Test that SSM source IPs must match the group's IP version.
///
/// IPv4 source IPs cannot be used with IPv6 multicast groups (and vice versa).
/// This test verifies the API rejects mismatched address families.
#[nexus_test]
async fn test_ssm_source_ip_version_mismatch(
    cptestctx: &ControlPlaneTestContext,
) {
    let client = &cptestctx.external_client;

    // Setup: create IPv6 multicast pool only
    ops::join3(
        create_default_ip_pools(client),
        create_project(client, PROJECT_NAME),
        create_multicast_ip_pool_v6(client, "ipv6-ssm-pool"),
    )
    .await;
    ensure_multicast_test_ready(cptestctx).await;
    ...
    let instance = instance_for_multicast_groups(
        cptestctx,
        PROJECT_NAME,
        "ssm-mismatch-instance",
        false,
        &[],
    )
    .await;

    // Try to join IPv6 group with IPv4 source IPs - should fail
    let join_url = format!(
        "/v1/instances/{}/multicast-groups/mismatch-group?project={PROJECT_NAME}",
        instance.identity.id
    );

    // Mismatched address families return 400 Bad Request
    let error: dropshot::HttpErrorResponseBody = NexusRequest::new(
        RequestBuilder::new(client, http::Method::PUT, &join_url)
            .body(Some(&InstanceMulticastGroupJoin {
                source_ips: Some(vec![
                    "10.0.0.1".parse::<IpAddr>().unwrap(), // IPv4 source
                ]),
                ip_version: Some(IpVersion::V6), // IPv6 group
            }))
            .expect_status(Some(StatusCode::BAD_REQUEST)),
    )
    .authn_as(AuthnMode::PrivilegedUser)
    ...
    assert_eq!(
        error.error_code,
        Some("InvalidRequest".to_string()),
        "Expected InvalidRequest for source/group IP version mismatch, got: {:?}",
        error.error_code
    );
}

/// Test that IPv6 source IPs cannot be used with IPv4 multicast groups.
///
/// This is the inverse of `test_ssm_source_ip_version_mismatch`.
#[nexus_test]
async fn test_ssm_ipv6_source_with_ipv4_group_fails(
    cptestctx: &ControlPlaneTestContext,
) {
    let client = &cptestctx.external_client;

    // Setup: create IPv4 multicast pool only
    ops::join3(
        create_default_ip_pools(client),
        create_project(client, PROJECT_NAME),
        create_multicast_ip_pool(client, "ipv4-ssm-pool"),
    )
    .await;
    ensure_multicast_test_ready(cptestctx).await;
    ...
    let instance = instance_for_multicast_groups(
        cptestctx,
        PROJECT_NAME,
        "ssm-inverse-mismatch-instance",
        false,
        &[],
    )
    .await;

    // Try to join IPv4 group with IPv6 source IPs - should fail
    let join_url = format!(
        "/v1/instances/{}/multicast-groups/inverse-mismatch-group?project={PROJECT_NAME}",
        instance.identity.id
    );
    ...
    let error: dropshot::HttpErrorResponseBody = NexusRequest::new(
        RequestBuilder::new(client, http::Method::PUT, &join_url)
            .body(Some(&InstanceMulticastGroupJoin {
                source_ips: Some(vec![
                    "2001:db8::1".parse::<IpAddr>().unwrap(), // IPv6 source
                ]),
                ip_version: Some(IpVersion::V4), // IPv4 group
            }))
            .expect_status(Some(StatusCode::BAD_REQUEST)),
    )
    .authn_as(AuthnMode::PrivilegedUser)
