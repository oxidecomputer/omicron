...
async fn test_cache_ttl_driven_refresh() {
    ...
    // Set short cache TTLs for testing (1 second for sled cache)
...
async fn test_backplane_cache_ttl_expiry() {
    ...
    // Set backplane cache TTL to 500ms (shorter than sled cache to test independently)
    config.pkg.background_tasks.multicast_reconciler.backplane_cache_ttl_secs =
        chrono::TimeDelta::milliseconds(500).to_std().unwrap();
    // Keep sled cache TTL longer to ensure we're testing backplane cache expiry
    config.pkg.background_tasks.multicast_reconciler.sled_cache_ttl_secs =
        chrono::TimeDelta::seconds(5).to_std().unwrap();
