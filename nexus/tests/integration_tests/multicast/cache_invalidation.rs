...
//! Integration test for multicast reconciler cache invalidation.
...
async fn test_sled_move_updates_multicast_port_mapping(
    ...
    let params = MulticastGroupCreate {
        identity: IdentityMetadataCreateParams {
            ...
            description: "Group for sled move test".to_string(),
        },
        multicast_ip: Some("224.0.1.200".parse::<IpAddr>().unwrap()),
        source_ips: None,
        pool: Some(omicron_common::api::external::NameOrId::Name(
            pool.identity.name.clone(),
        )),
        mvlan: None,
    };
    ...
    // Create instance and attach to multicast group
    ...
    verify_inventory_based_port_mapping(cptestctx, &instance_uuid)
        .await
        .expect("initial port mapping verification");
    ...
    let original_sp = original_inventory
        ...
        .map(|(_, sp)| sp)
        .expect("found SP for sled in original inventory");
    ...
    let backplane =
        dpd.backplane_map().await.expect("fetch backplane map").into_inner();
    ...
    datastore
        ...
        .await
        .expect("insert new inventory collection");
...
///
/// - Start test server with sled_cache_ttl = 2 seconds
...
/// - Wait for TTL to expire (sleep 3 seconds)
...
async fn test_cache_ttl_driven_refresh() {
    ...
    let params = MulticastGroupCreate {
        identity: IdentityMetadataCreateParams {
            name: GROUP_NAME.parse().unwrap(),
            description: "Group for TTL refresh test".to_string(),
        },
        multicast_ip: Some("224.0.1.210".parse::<IpAddr>().unwrap()),
        source_ips: None,
        pool: Some(omicron_common::api::external::NameOrId::Name(
            pool.identity.name.clone(),
        )),
    ...
    // Wait for cache TTL to expire (sled_cache_ttl = 2 seconds)
    // Sleep for 3 seconds to ensure TTL has expired
    ...
    .await
    .expect("DPD should be updated with new rear port after TTL expiry");
...
async fn test_backplane_cache_ttl_expiry() {
    ...
    let params = MulticastGroupCreate {
        identity: IdentityMetadataCreateParams {
            name: GROUP_NAME.parse().unwrap(),
            description: "Group for backplane TTL test".to_string(),
        },
        multicast_ip: Some("224.0.1.230".parse::<IpAddr>().unwrap()),
        source_ips: None,
        pool: Some(omicron_common::api::external::NameOrId::Name(
            pool.identity.name.clone(),
        )),
    ...
    // Wait for backplane cache TTL to expire (1 second) but not sled cache (10 seconds)
    // Sleep for 2 seconds to ensure backplane TTL has expired
    ...
    verify_inventory_based_port_mapping(&cptestctx, &instance_uuid)
        .await
        .expect("port mapping after backplane cache TTL expiry");
