...
# For networking resources, when a silo restricts networking, only silo admins can modify/create
# This applies to resources that use the InProjectNetworking polar snippet

# Networking modify permissions with silo restriction logic
# Allow silo admins always (override any restrictions)
has_permission(actor: AuthenticatedActor, "modify", vpc: Vpc) if
    has_role(actor, "admin", vpc.project.parent_silo);

has_permission(actor: AuthenticatedActor, "modify", router: VpcRouter) if
    has_role(actor, "admin", router.vpc.project.parent_silo);

has_permission(actor: AuthenticatedActor, "modify", subnet: VpcSubnet) if
    has_role(actor, "admin", subnet.vpc.project.parent_silo);

has_permission(actor: AuthenticatedActor, "modify", gateway: InternetGateway) if
    has_role(actor, "admin", gateway.vpc.project.parent_silo);

# Allow project collaborators only if silo doesn't restrict networking
has_permission(actor: AuthenticatedActor, "modify", vpc: Vpc) if
...
# Networking create_child permissions with silo restriction logic
# Allow silo admins always (override any restrictions)
