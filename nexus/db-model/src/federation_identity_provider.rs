...
use openidconnect::core::{
    CoreJsonWebKeySet, CoreJsonWebKeyType, CoreJsonWebKeyUse,
};
...
fn parse_signing_keys(value: Value) -> Result<Value, Error> {
    ...
    for key in jwks.keys() {
        let kid = key.key_id().map(|kid| kid.as_str()).ok_or_else(|| {
        ...
        if key
            .key_use()
            .is_some_and(|usage| *usage != CoreJsonWebKeyUse::Signature)
        {
...
mod tests {
    ...
    fn federation_signing_keys() {
        ...
        for (field, value) in [("kid", json!("")), ("use", json!("enc"))] {
