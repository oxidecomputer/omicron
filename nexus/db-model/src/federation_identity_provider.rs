    ...
    pub fn new(
        ...
        let verification_type = match params.verification_type {
            api::FederationVerificationType::OidcDiscovery => {
                let url = params.discovery_url.as_deref().ok_or_else(|| {
                    Error::invalid_request(
                        "oidc_discovery requires discovery_url",
                    )
                })?;
                ...
                let url = url::Url::parse(url).map_err(|_| {
                    Error::invalid_request("invalid discovery_url")
                })?;
                if url.scheme() != "https"
                ...
                {
                    return Err(Error::invalid_request(
                        "discovery_url must be HTTPS without credentials or a fragment",
                    ));
                }
                "oidc_discovery"
            }
            api::FederationVerificationType::StaticJwks => {
                if params.discovery_url.is_some() {
                    return Err(Error::invalid_request(
                        "static_jwks does not accept discovery_url",
                    ));
                }
...
mod tests {
    ...
    fn discovery() -> api::FederationIdentityProviderCreate {
        api::FederationIdentityProviderCreate {
            ...
            discovery_url: Some(
                "https://metadata.example.com/.well-known/openid-configuration"
                    .into(),
            ),
            signing_keys: None,
        }
    ...
    fn federation_provider_configuration() {
        ...
        for url in [
            ...
            let mut params = discovery();
            params.discovery_url = Some(url.into());
            assert!(
                FederationIdentityProvider::new(Uuid::new_v4(), params)
                    .is_err(),
                "{url}"
            );
        }
