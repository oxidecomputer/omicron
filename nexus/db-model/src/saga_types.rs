...
/// Newtype wrapper around String that's used to represent SEC generations
#[derive(
    Clone,
    Debug,
    Display,
...
#[repr(transparent)]
#[display("{0}")]
pub struct SecGeneration(pub String);

NewtypeFrom! { () pub struct SecGeneration(String); }
NewtypeDeref! { () pub struct SecGeneration(String); }

impl SecGeneration {
    /// Generates an SEC generation suitable for Nexus
    ///
    /// Besides the SecGeneration itself, this function returns a boolean
    /// indicating whether a random value was used rather than the current
    /// timestamp because the current time appeared unreasonable.
    pub fn generate() -> (SecGeneration, bool) {
    ...
    /// Generates an SEC generation based on a fixed value
    ///
    /// This is very unlikely to collide with an SEC generation generated in any
    /// other way (i.e., with `generate_time()` or `generate()`), but may be the
    /// same as any SEC generation generated by this function.
    ///
    /// This is only intended for testing.
    pub fn random() -> SecGeneration {
        SecGeneration(
...
impl<DB> ToSql<sql_types::Text, DB> for SecGeneration
where
    DB: Backend,
...
impl<DB> FromSql<sql_types::Text, DB> for SecGeneration
where
    DB: Backend,
...
pub struct Saga {
    ...
    sec_generation: Option<SecGeneration>,
}

impl Saga {
    pub fn new(
        creator: SecId,
        sec_generation: SecGeneration,
