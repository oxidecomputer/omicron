...
impl From<SbdataReconfigurator> for BundleData {
    fn from(_: SbdataReconfigurator) -> Self {
...
impl From<SbdataHostInfo> for BundleData {
    fn from(row: SbdataHostInfo) -> Self {
        BundleData::HostInfo(row.into_sled_selection())
    }
}
...
impl From<SbdataSledCubbyInfo> for BundleData {
    fn from(_: SbdataSledCubbyInfo) -> Self {
...
impl From<SbdataSpDumps> for BundleData {
    fn from(_: SbdataSpDumps) -> Self {
        BundleData::SpDumps
    }
}
...
impl TryFrom<SbdataEreports> for BundleData {
    type Error = omicron_common::api::external::Error;

    /// Reconstruct the `BundleData::Ereports` variant from the DB row.
    ///
    /// Returns an error if the stored start/end times are inconsistent
    /// (e.g. start > end due to corrupt data).
    fn try_from(row: SbdataEreports) -> Result<Self, Self::Error> {
        let mut filters = EreportFilters::new();
        if let Some(t) = row.start_time {
            filters = filters.with_start_time(t)?;
        }
        if let Some(t) = row.end_time {
            filters = filters.with_end_time(t)?;
        }
        Ok(BundleData::Ereports(
            filters
                .with_serials(row.only_serials)
                .with_classes(row.only_classes),
        ))
    }
}
