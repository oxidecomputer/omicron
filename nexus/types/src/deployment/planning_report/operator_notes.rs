...
use crate::deployment::PlanningNexusGenerationBumpReport;
...
use super::ZoneWaitingToExpunge;
...
impl PlanningOperatorNotes {
    pub(super) fn new(report: &PlanningReport) -> Self {
        ...
        me.push_nexus_generation_bump_notes(nexus_generation_bump);
    ...
    fn push_add_notes(&mut self, add: &PlanningAddStepReport) {
        ...
        let sleds_with_ntp_problems = {
            let set1 = sleds_without_ntp_zones_in_inventory
                .union(sleds_without_zpools_for_ntp_zones)
                .collect::<BTreeSet<_>>();
            let set2 = sleds_waiting_for_ntp_zone
                .union(sleds_missing_ntp_zone)
                .collect::<BTreeSet<_>>();
            set1.union(&set2).count()
        };
        if sleds_with_ntp_problems > 0 {
            self.notes.push(format!(
                "{sleds_with_ntp_problems} sled{} waiting for NTP",
                pluralize_s(sleds_with_ntp_problems),
            ));
        }
    ...
    fn push_zone_updates_notes(
        ...
        let PlanningZoneUpdatesStepReport {
            unsafe_zones,
            waiting_zones,
            // this is a summary point already covered by details elsewhere
            waiting_on: _,
        ...
        {
            let mut emitted_nexus = false;
            for kind in waiting_zones.values() {
                match kind {
                    ZoneWaitingToExpunge::Nexus { .. } => {
                        if !emitted_nexus {
                            self.notes.push(
                                "waiting for handoff from old \
    ...
    fn push_nexus_generation_bump_notes(
        &mut self,
        nexus_generation_bump: &PlanningNexusGenerationBumpReport,
    ) {
        match nexus_generation_bump {
            PlanningNexusGenerationBumpReport::NothingToReport => (),
            PlanningNexusGenerationBumpReport::WaitingOn(_) => {
                // This is "an update is in progress"; nothing particular to
                // note that an operator can act upon.
            }
            PlanningNexusGenerationBumpReport::BumpingGeneration(_) => {
                self.notes.push(
                    "initiating handoff from old version to new".to_string(),
                );
            }
        }
    }
