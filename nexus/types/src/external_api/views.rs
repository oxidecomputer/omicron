...
impl Into<OxqlTable> for oxql_types::Table {
    fn into(self) -> OxqlTable {
        let timeseries: Vec<oxql_types::Timeseries> =
            self.timeseries().cloned().collect();
        OxqlTable { name: self.name, timeseries }
    }
}
