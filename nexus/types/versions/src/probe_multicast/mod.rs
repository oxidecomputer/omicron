...
//! at probe creation. It also reshapes `MulticastGroupMember` from a single
//! `instance_id` field to a `kind` / `parent_id` pair so a member may be
//! parented by either an instance or a probe.
