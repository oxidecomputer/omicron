...
/// A router configuration used by control-plane services, with its priority
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, PartialEq)]
pub struct ControlPlaneRouterConfiguration {
    /// ID of the router configuration
    pub router_configuration_id: Uuid,

    /// Priority of the router configuration within the control-plane list.
    /// Priorities are unique within the list.
    pub priority: u16,
}
...
/// Assignment of a router configuration to control-plane services
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, PartialEq)]
pub struct ControlPlaneRouterConfigurationEntry {
    /// A name or id of the router configuration to assign
    pub router_configuration: NameOrId,

    /// Priority of the router configuration within the control-plane list.
    /// Priorities must be unique within the list.
    pub priority: u16,
}

/// Full replacement of the set of router configurations used by
/// control-plane services
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, PartialEq)]
pub struct ControlPlaneRouterConfigurationsUpdate {
    /// The complete new set of router configurations for control-plane
    /// services. An empty set means services get no external egress; to
    /// restore the default router, assign a built-in per-switch
    /// configuration ("default-switch0" or "default-switch1").
    pub configurations: Vec<ControlPlaneRouterConfigurationEntry>,
}
