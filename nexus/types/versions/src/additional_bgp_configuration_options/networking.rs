...
//! Networking types for the `ADDITIONAL_BGP_CONFIGURATION_OPTIONS` version.
//!
//! Changes in this version:
//!
//! * New [`BgpConfigUpdate`] type to allow updating a BGP configuration's
//!   `max_paths` and `bgp_announce_set_id` fields without deleting and
//!   recreating the object.
//! * [`BgpPeer`] gains a `src_addr` field: an optional local source IP
//!   address for initiating BGP sessions.
//! * New versions of [`BgpPeerConfig`], [`SwitchPortSettingsCreate`], and
//!   [`SwitchPortSettings`] that include the updated [`BgpPeer`].
    ...
    for SwitchPortSettingsCreate
{
    fn from(
        ...
        Self {
            ...
            routes: value.routes,
            bgp_peers: value.bgp_peers.into_iter().map(Into::into).collect(),
