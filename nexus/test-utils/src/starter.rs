...
impl<'a, N: NexusServer> ControlPlaneStarter<'a, N> {
    ...
    pub async fn start_gateway(
        ...
        if switch_slot == SwitchSlot::Switch1 {
        ...
        .await;
        self.gateway.insert(switch_slot, gateway);
    }
        ...
        .init_with_steps(
            vec![
                ...
                (
                    ...
                    Box::new(|builder| {
                        builder.start_dendrite(SwitchSlot::Switch0).boxed()
                    }),
                ),
                (
                    "start_mgd_switch0",
...
///
/// This is not exposed outside this package because we want to ensure that
/// tests use the setup script rather than trying to create their own seed
/// tarballs. This may need to be revisited if circumstances change.
#[derive(Clone, Debug)]
pub(crate) enum PopulateCrdb {
