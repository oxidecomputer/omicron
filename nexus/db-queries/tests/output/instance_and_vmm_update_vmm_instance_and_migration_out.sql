  ...
  migration_out_updated
    AS (
      UPDATE
        migration
      SET
        source_state = $11, time_source_updated = $12, source_gen = $13
      WHERE
        (migration.id = $14 AND migration.source_propolis_id = $15) AND migration.source_gen < $16
      RETURNING
        id
    ),
...
FROM
  vmm_result, instance_result, migration_out_result
