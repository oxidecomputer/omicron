    ...
    AS (
      SELECT
        EXISTS(
          ...
          WHERE
            dtc.origin_snapshot = $4 AND d.project_id = $5 AND d.time_deleted IS NULL AND d.id != $6
        )
        OR EXISTS(
            SELECT
              1
            FROM
              physical_provisioning_resource AS ppr INNER JOIN snapshot AS s ON s.id = ppr.id
            WHERE
              s.id = $7
              AND s.project_id = $8
      ...
      SELECT
        ...
        OR EXISTS(
            SELECT
              1
            FROM
              physical_provisioning_resource AS ppr
              INNER JOIN snapshot AS s ON s.id = ppr.id
              INNER JOIN project AS p ON s.project_id = p.id
            WHERE
              s.id = $11
              AND p.silo_id = (SELECT id FROM parent_silo)
              AND s.time_deleted IS NULL
  ...
  unused_cte_arm
    AS (
      DELETE FROM
        physical_provisioning_resource
      WHERE
        physical_provisioning_resource.id = $13 AND (SELECT do_update.update FROM do_update LIMIT 1)
      RETURNING
    ...
    AS (
      ...
      SET
        ...
        physical_writable_disk_bytes
          = physical_provisioning_collection.physical_writable_disk_bytes
          - CASE WHEN physical_provisioning_collection.id = $14 THEN $15 ELSE $16 END,
        physical_zfs_snapshot_bytes
          = physical_provisioning_collection.physical_zfs_snapshot_bytes
          - CASE WHEN physical_provisioning_collection.id = $17 THEN $18 ELSE $19 END,
        physical_read_only_disk_bytes
          = physical_provisioning_collection.physical_read_only_disk_bytes
          - CASE
            WHEN physical_provisioning_collection.id = $20
            THEN CASE WHEN (SELECT already_referenced FROM dedup_in_project) THEN 0 ELSE $21 END
            ELSE CASE WHEN (SELECT already_referenced FROM dedup_in_silo) THEN 0 ELSE $22 END
            END
      WHERE
