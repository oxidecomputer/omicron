  ...
  unused_cte_arm
    AS (
      DELETE FROM
        physical_provisioning_resource
      WHERE
        physical_provisioning_resource.id = $6 AND (SELECT do_update.update FROM do_update LIMIT 1)
      RETURNING
  ...
  physical_provisioning_collection
    AS (
      ...
      SET
        ...
        cpus_provisioned = physical_provisioning_collection.cpus_provisioned - $7,
        ram_provisioned = physical_provisioning_collection.ram_provisioned - $8
      WHERE
