...
use crate::db::queries::external_subnet::INSTANCE_STILL_ATTACHED_SENTINEL;
use crate::db::queries::external_subnet::NO_LINKED_POOL_CONTAINS_REQUESTED_SUBNET_SENTINEL;
use crate::db::queries::external_subnet::SUBNET_OVERLAPS_EXISTING_SENTINEL;
...
impl DataStore {
        ...
        insert_subnet_pool_member_query(&member)?
            ...
            .map_err(|e| match e {
                DieselError::NotFound => Error::invalid_request(&format!(
                    ...
                    member of external subnet pool pool with ID '{}'",
                    params.subnet,
                    authz_pool.id(),
                )),
    ...
    pub async fn create_external_subnet(
        ...
        insert_external_subnet_query(silo_id, &authz_project.id(), params)
            .get_result_async(&*self.pool_connection_authorized(opctx).await?)
            .await
            .map_err(|e| match &e {
                DieselError::DatabaseError(
                    ...
                    if is_bool_parse_error(
                        msg,
                        SUBNET_OVERLAPS_EXISTING_SENTINEL,
                    ) {
                        Error::invalid_request(
                            "The requested IP subnet overlaps with \
                            an existing External Subnet",
                        )
...
mod tests {
    ...
    async fn cannot_insert_subnet_pool_member_with_overlapping_ip_subnet() {
        ...
        let subnet = IpNet::V4("10.0.1.0/16".parse().unwrap());
        let params = SubnetPoolMemberCreate {
            pool: pool_id.clone(),
            subnet,
            min_prefix_length: 16,
        ...
        assert_eq!(message.external_message(), "foo",);
    }
