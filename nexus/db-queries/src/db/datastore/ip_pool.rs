...
impl ServiceIpPools {
    /// Return the first IP Pool appropriate for a range, based on its version.
    pub fn first_pool_for_range(
        &self,
        range: &IpRange,
    ) -> Option<&ServiceIpPool> {
        let pools = if range.first_address().is_ipv4() {
            &self.ipv4
        } else {
            &self.ipv6
        };
    ...
    /// Return the first IP Pool appropriate for an IP version.
    pub fn first_pool_for_version(
        &self,
        version: IpVersion,
    ) -> Option<&IpPool> {
...
impl DataStore {
    ...
    pub async fn ip_pools_service_lookup_both_versions(
        ...
        let ipv6 = self
            ...
            .await?;
        Ok(ServiceIpPools {
            ipv4: vec![ServiceIpPool { authz_pool: ipv4.0, db_pool: ipv4.1 }],
            ipv6: vec![ServiceIpPool { authz_pool: ipv6.0, db_pool: ipv6.1 }],
        })
    }
