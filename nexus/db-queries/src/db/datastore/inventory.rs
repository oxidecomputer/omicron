                ...
                for (baseboard_id, sp) in &collection.sps {
                    let selection = db::schema::hw_baseboard_id::table
                        .select((
                            ...
                            sp.slot0_sw_caboose.map(|u| {
                                u.into_sql::<diesel::sql_types::Uuid>()
                            }),
                            sp.slot1_sw_caboose.map(|u| {
                                u.into_sql::<diesel::sql_types::Uuid>()
                            }),
                        ))
                        .filter(
                    ...
                    .into_columns((
                        ...
                        sp_dsl::power_state,
                        sp_dsl::slot0_sw_caboose_id,
                        sp_dsl::slot1_sw_caboose_id,
                    ))
                    .execute_async(&conn)
                    .await?;
                    ...
                    let (
                        ...
                        _hubris_archive_id,
                        _power_state,
                        _slot0_sw_caboose_id,
                        _slot1_sw_caboose_id,
