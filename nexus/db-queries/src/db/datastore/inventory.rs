...
mod test {
    ...
    // database. The blueprint system only generates zones with a single
    // external IP today, so we construct the multi-IP zone by hand.
    #[tokio::test]
    async fn test_zone_with_multiple_external_ips_reads_back() {
        let logctx =
            dev::test_setup_log("zone_with_multiple_external_ips_reads_back");
        let db = TestDatabase::new_with_datastore(&logctx.log).await;
        let (opctx, datastore) = (db.opctx(), db.datastore());
