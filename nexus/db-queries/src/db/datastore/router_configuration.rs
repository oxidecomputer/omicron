...
impl DataStore {
            ...
            .transaction(&conn, async |conn| {
                ...
                let cp_in_use = diesel::dsl::select(diesel::dsl::exists(
                    cp_link_dsl::control_plane_router_configuration.filter(
                        cp_link_dsl::router_configuration_id.eq(id),
                    ),
                ))
                .get_result_async::<bool>(&conn)
                .await?;
    ...
    /// priority order.
    ///
    /// No rows means no external routing for services.
    ...
    ///
    /// Empty `links` clears the list, disabling external routing for services.
