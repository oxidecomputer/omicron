...
impl DataStore {
    ...
    pub async fn silo_router_configurations_list_all(
        ...
        while let Some(p) = paginator.next() {
            ...
            paginator = p
                .found_batch(&batch, &|l| (l.silo_id, i32::from(*l.priority)));
            links.extend(batch);
        }
        Ok(links)
    }
    ...
    /// List every live service NIC id. The router-lists task uses this to
    /// give service ports the control-plane list explicitly, so ports it
    /// cannot account for fail closed (empty list) instead of inheriting
    /// the control-plane list.
