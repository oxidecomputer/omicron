...
/// - Look for the smallest free address (or chunk of one) in an IP Pool Range.
/// - Join the `external_ip.ip` column with `external_ip.ip + 1`, i.e., the
///  "next address", and take that one if it's free and is within the IP Pool
///  Range itself.
///
...
impl NextExternalIp {
    ...
    fn push_next_external_ip_cte_select_clauses<'a>(
        ...
        // Generate or select the first / last ports.
        //
        // If this is an SNAT IP address, we generate them from the following
        // subquery. If not, we can insert constants, since this record takes
