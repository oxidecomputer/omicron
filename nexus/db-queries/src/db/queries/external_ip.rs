...
impl QueryFragment<Pg> for NextExternalIp {
    fn walk_ast<'a>(
        ...
        //
        // Note that Floating IPs are created without a parent, so this check
        // cannot apply. Instead, we check that at attach time.
        match self.ip.kind() {
            IpKind::SNat | IpKind::Ephemeral => {
                if let Some(parent_id) = self.ip.parent_id().as_ref() {
                    ...
                    panic!(
                        "SNAT / Ephemeral addresses are always expected to \
                        have a parent, but no parent ID exists"
                    );
                }
            }
            IpKind::Floating => {}
