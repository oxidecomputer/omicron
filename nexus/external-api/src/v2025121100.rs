            ...
            PrivateIpStack::DualStack { .. } => {
                return Err(external::Error::invalid_request(format!(
                    "The network interface with ID '{}' is \
                        a dual-stack NIC. The version of the \
                        client being used is unable to fully \
...
impl TryFrom<InstanceNetworkInterfaceUpdate>
    for params::InstanceNetworkInterfaceUpdate
{
    type Error = external::Error;

    fn try_from(
        _value: InstanceNetworkInterfaceUpdate,
    ) -> Result<Self, Self::Error> {
        todo!()
    }
}
