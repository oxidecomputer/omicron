...
//!
//! * **\[deployment\] blueprint**: describes what software _should_ be running
...
//!   realize the same blueprint concurrently without stomping on each other.
//!   (For example, it's not specific enough to say "there should be one more
...
//!   two consecutive blueprints the version of every CockroachDB node changed,
//!   then concurrent blueprint execution might try to update them all at once,
//!   bringing the whole Cockroach cluster down.  In this case, we need to use a
//!   sequence of blueprints that each only updates one node at a time to ensure
//!   that the system keeps working.
//!
//!   At any given time, the system has exactly one _goal_ blueprint.  The
...
//! (target) blueprint is consistent with the current policy.  If not, the task
//! generates a new blueprint that _is_ consistent with the current policy and
...
//! winning blueprint before trying to change it again -- otherwise two Nexus
//! instances might fight over two equivalent blueprints.)
//!
...
//! target blueprint.  If not, it executes operations to bring reality into line
//! with the blueprint.  This means provisioning new zones, removing old zones,
