...
impl Client {
    ...
    /// Returns the sled address behind each subnet
    /// [`Self::derive_underlay_subnets_from_prefixes`] returns, and the
    /// same caveat applies: callers must probe for the sleds they
    /// expect to find.
