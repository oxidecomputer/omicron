...
use loopback_ip_mgr::IpAllocation;
...
pub struct MgdInstance {
    ...
    /// RAII guard keeping a loopback IP allocated for the BGP dispatcher
    /// address. Held here so the address is released when the instance is
    /// cleaned up. `None` when using an address that is always present (e.g.
    /// 127.0.0.1).
    pub bgp_loopback_allocation: Option<IpAllocation>,
}
