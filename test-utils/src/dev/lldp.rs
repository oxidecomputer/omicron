...
impl LldpdInstance {
    pub async fn start(
        ...
        if port == 0 {
            port = discover_port(temp_dir.join("lldpd_stdout").to_string())
                .await
                .with_context(|| {
                    format!(
                        "failed to discover lldpd port from files in {temp_dir}"
                    )
                })?;
        }
    ...
    fn drop(&mut self) {
        if self.child.is_some() || self.data_dir.is_some() {
            ...
            if let Some(path) = self.data_dir.take() {
                eprintln!("WARN: lldpd temporary directory leaked: {path}");
            }
        }
    }
}
...
"1","unit":"api-server"}"#;
        ...
        assert_eq!(
            find_lldpd_port_in_log(file.path().to_string()).await.unwrap(),
            EXPECTED_PORT
        );
    }
}
