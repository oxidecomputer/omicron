...
** bootstrap agent: Launches sled agent and OXC.
...
To **run the system:** there are two executables: the `nexus` (which
...
`nexus` requires a configuration file to run.  You can use
...
IP:port for itself, and the IP:port of `nexus`'s _internal_
...
----
$ cargo build
$ cargo run --bin=oxcp_package
...
# Your ./out directory should now be populated.
----

The manifests are currently hardcoded to expect installation within the
`/opt/oxide/oxcp` directory. We can copy them to their expected location with
the following:

[source,text]
----
$ mkdir -p /opt/oxide/oxcp
$ cp -r ./out/* /opt/oxide/oxcp
----

[NOTE]
====
Running some commands on Helios may require privileged access.  If any
subsequent commands result in "access denied", try the same command, but
prefixed with `pfexec`.

For Linux users, this is the Helios equivalent of `sudo`.
====

Once this is complete, we can unpack and initialize the bootstrap agent.
This should unpack and initialize all other services.

[source,text]
----
$ tar -xvzf /opt/oxide/oxcp/bootstrap_agent.tar -C /opt/oxide/oxcp/bootstrap_agent
$ svccfg import -V /opt/oxide/oxcp/bootstrap_agent/smf/bootstrap_agent/manifest.xml
----
