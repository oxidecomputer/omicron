...
** sled agent: manages a single compute server in an Oxide fleet
...
`sled_agent` (which is a simulation of the component that will manage an Oxide
server).
...
identifying itself (this would be a uuid for the server it's managing), an
...
NOTE: Currently, `sled_agent` will crash if `oxide_controller` is not already
running.
