...
** disks: CR + attach/detach
** racks: list + get
** servers: list + get
* architecture:
** oxide controller: manages a fleet of Oxide racks
** server controller: manages a single compute server in an Oxide fleet
...
You can build and run the whole test suite with `cargo test`.  The test suite
runs cleanly and should remain clean.

There are two executables: the `oxide_controller` (which is a real prototype
backed by an in-memory store; this component manages an Oxide fleet), and the
`server_controller` (which is a simulation of the component that will manage an
Oxide server).
...
`server_controller` only takes configuration on the command line.  Run it with a
uuid identifying itself (this would be a uuid for the server it's managing), an
...
[source,text]
----
$ cargo run --bin=server_controller -- $(uuidgen) 127.0.0.1:12345 127.0.0.1:12221
...
Jun 02 12:21:50.989 INFO listening, local_addr: 127.0.0.1:12345, component: dropshot
----

NOTE: Currently, `server_controller` will crash if `oxide_controller` is not
already running.
...
|Yes
|Specifies that the server should bind to the given IP address and TCP port for
the **internal** API (i.e., the one used by the server controller).  In general,
...
- **Oxide Controller**.  This is an HTTP-agnostic model of the resources in a
  rack.
