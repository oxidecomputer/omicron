...
To **run Omicron:** there are two executables: `nexus` (which is a real prototype backed by an in-memory store; this component manages an Oxide fleet), and the `sled_agent` (which is a simulation of the component that will manage a single sled).
