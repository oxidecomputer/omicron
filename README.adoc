...
=== tombi for TOML formatting and linting

This repository uses https://tombi-toml.github.io/tombi/docs/[tombi] for formatting and linting TOML files. Among other things, tombi ensures that `Cargo.toml` dependency lines are always in sorted order. Install it with `cargo xtask download tombi`.

* Lint TOML files with `tombi lint`.
* Autoformat TOML files with `tombi fmt`.
...
https://github.com/oxidecomputer/dropshot-api-manager[`dropshot-openapi-manager`].
