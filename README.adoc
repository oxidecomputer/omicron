...
This repository uses https://tombi-toml.github.io/tombi/docs/[tombi] for formatting TOML files. Among other things, tombi ensures that `Cargo.toml` dependency lines are always in sorted order. Install it with `cargo xtask download tombi`, then autoformat TOML files with `tombi fmt`.
