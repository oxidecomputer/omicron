...
async fn ensure_zpool_dataset_is_encrypted(
    ...
    if dataset_exists(&encrypted_dataset).await? {
        info!(log, "Dataset already has encrypted variant");
        return Ok(());
    }
    info!(log, "Dataset has not yet been encrypted");

    let snapshot_of = |dataset: &str| format!("{dataset}@migration");

    // Gather full names to act on the following datasets.
    //
    // The following are documented with examples for the "cockroachdb" dataset:

    // The original, unencrypted dataset
    // /oxp_<UUID>/cockroachdb
    ...
    // copying.
    // /oxp_<UUID>/cockroachdb@migration
    let unencrypted_dataset_snapshot = snapshot_of(&unencrypted_dataset);
    // A "temporary" name for the encrypted target dataset.
    // /oxp_<UUID>/crypt/cockroachdb-tmp
    let encrypted_dataset_tmp = format!("{}-tmp", encrypted_dataset);
    // A snapshot that is automatically generated by "zfs receive".
    // /oxp_<UUID>/crypt/cockroachdb-tmp@migration
    let encrypted_dataset_tmp_snapshot = snapshot_of(&encrypted_dataset_tmp);

    // ---
    //
    // NOTE: This is where we begin issuing ZFS commands that modify the
    // underlying system.
    //
    // ---
    ...
    info!(log, "Encrypted dataset fully created");
...
async fn zfs_transfer_to_zoned_dataset(
    from: &str,
    to: &str,
) -> Result<(), DatasetEncryptionMigrationError> {
    let mut command = tokio::process::Command::new(illumos_utils::zfs::ZFS);
    ...
    let receiver_cmd = command
        .args(&["receive", "-o", "mountpoint=/data", "-o", "zoned=on", to])
        .stdin(sender_stdout)
        .stderr(Stdio::piped());
