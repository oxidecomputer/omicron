...
impl<'a> UpdatePlanBuilder<'a> {
    ...
    fn add_trampoline_artifact(
        ...
        let gimlet_unique_artifact_id = ArtifactId {
            name: format!("{}-gimlet", artifact_id.name),
            version: artifact_id.version.clone(),
            kind: KnownArtifactKind::Trampoline.into(),
        };
        ...
        self.record_extracted_artifact(
            gimlet_unique_artifact_id,
            gimlet_phase_1_data,
            ArtifactKind::TRAMPOLINE_PHASE_1,
            Some(Board("gimlet-host".to_string())),
            None,
            self.log,
        )?;
        self.record_extracted_artifact(
            cosmo_unique_artifact_id,
            cosmo_phase_1_data,
            ArtifactKind::TRAMPOLINE_PHASE_1,
            Some(Board("cosmo-host".to_string())),
            None,
            self.log,
        )?;
        self.record_extracted_artifact(
