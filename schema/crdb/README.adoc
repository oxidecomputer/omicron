...
==== Non-transactional schema updates

By default, each `up.sql` file is run in a transaction. However, in some
rare cases, schema updates may need to be done outside a transaction context.
To opt into this, add a `-- NON-TRANSACTIONAL` comment on its own as
the first line of the file.

Non-transactional updates are a last-resort feature, and should only be used when
strictly necessary.

Currently, the only non-transactional schema update is a `SET CLUSTER SETTING`.

===== Idempotence of non-transactional updates

Transactional schema updates are required to be idempotent, but only with regard
to other instances of Nexus applying the same `up.sql` file. On the other hand,
non-transactional schema updates are susceptible to version TOCTOU issues, and
must be idempotent across _other_ schema updates.

What does this mean? Consider this (admittedly contrived) example: within a
schema migration, `up1.sql` adds a column and `up2.sql` removes that column.
With transactional schema updates, it is guaranteed that after both migrations
have been applied, the column is not present. But if `up1.sql` is marked
non-transactional, in the middle of one copy of Nexus applying it, a different
copy of Nexus might apply it, move on to `up2.sql`, and apply that as well. So
whether the column exists or not is indeterminate. This means that typical
schema update statements cannot be marked non-transactional.

In practice, the only non-transactional schema updates are likely to be special
situations like cluster-wide settings.
