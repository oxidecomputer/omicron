...
* Update `nexus/db-model/src/schema_version.rs`:
...
* Optional: check some of this work by running `cargo nextest run -p
  nexus-db-model -- schema_versions`.  This is recommended because if you get
...
same `NEW_VERSION`:** all you have to do is revisit
`nexus/db-model/src/schema_version.rs`.  In this situation, `OLD_VERSION` has
changed, so your `NEW_VERSION` will also have changed.  You'll need to:

* Make sure `SCHEMA_VERSION` reflects your new `NEW_VERSION`.
* Make sure the `KNOWN_VERSIONS` entry that you added reflects your new `NEW_VERSION` and still appears at the top of the list (logically after the new version that came in from "main").
