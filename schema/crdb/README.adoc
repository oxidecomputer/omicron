...
Adding a new variant to an enum is straightforward: `ALTER TYPE your_type ADD
VALUE IF NOT EXISTS your_new_value AFTER some_existing_value` (or `... BEFORE
some_existing_value`) (e.g., the `add-management-gateway-producer-kind`
migration).
