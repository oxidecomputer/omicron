...
** *Delivery attempts*:
*** `up12.sql` creates the enum `omicron.public.webhook_event_delivery_result`,
representing the potential outcomes of a webhook delivery attempt.
*** `up13.sql` creates the table `omicron.public.webhook_event_delivery_attempt`,
...
`omicron.public.webhook_event_delivery_attempt`, for looking up all attempts to
