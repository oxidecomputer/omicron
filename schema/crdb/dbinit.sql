...
/*
 * A policy for a single multirack setup
 *
 * We currently tie this policy to a rack, as we don't yet support multirack.
 * Multiple parts of this schema are going to have to change to support
 * multirack, so we add one more for now.
 */
CREATE TABLE IF NOT EXISTS omicron.public.policy (
