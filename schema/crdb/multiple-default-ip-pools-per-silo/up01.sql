...
--
-- Previously, a silo could only have one default IP pool. With this change,
-- a silo can now have up to 4 default pools: one for each combination
-- of pool_type (unicast/multicast) and ip_version (v4/v6).
--
-- For unicast-only use today, this allows separate default pools for IPv4 and
