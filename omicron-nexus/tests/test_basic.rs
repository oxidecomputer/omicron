...
use omicron_common::api::IdentityMetadataUpdateParams;
...
async fn test_projects() {
     ...
     */
    let expected_projects: Vec<&ProjectView> = initial_projects
        .iter()
    ...
    let mut response = client
        ...
        .await
        .expect("expected success");
    let project: ProjectView = read_json(&mut response).await;
...
async fn test_projects_list() {
    ...
    for _ in 0..nprojects {
        ...
        let project = objects_post::<_, ProjectView>(
            &client,
            &projects_url,
            create_params,
        )
    ...
    let found_projects_by_name =
        iter_collection::<ProjectView>(&client, projects_url, "", 99).await.0;
     ...
     * the same as above.
     */
    let found_projects_by_name = iter_collection::<ProjectView>(
        &client,
        projects_url,
     ...
     */
    let mut found_projects_by_name = iter_collection::<ProjectView>(
        &client,
        projects_url,
     ...
     */
    let found_projects_by_id = iter_collection::<ProjectView>(
        &client,
        projects_url,
...
async fn projects_list(
    ...
    objects_list_page::<ProjectView>(client, projects_url).await.items
}
...
async fn project_get(
    ...
    object_get::<ProjectView>(client, project_url).await
}
...
async fn sleds_list(
    ...
    objects_list_page::<SledView>(client, sleds_url).await.items
}
