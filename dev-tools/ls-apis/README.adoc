...
This tool verifies these properties.  If you add a new API or a new dependency that breaks these constraints, you will have to fix that before you can land the change to Omicron.
...
* `+missing field `versioned_how_reason`+`: this probably means that you added an API with `versioned_how = "client"`, but did not add a `version_how_reason`.  This field is required.  It's a free-text field that explains why we couldn't use server-side versioning for this API.
