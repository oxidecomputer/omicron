...
* The developer-maintained `package-manifest.toml`, which defines exactly what winds up on a deployed system
...
. It loads `package-manifest.toml` and uses that to determine which Git commits are used to deploy components from all of the relevant Cargo workspaces.
...
An _embedded component_ is such a library, tracked as an API consumer in its own right:
...
* `include-non-dag`: show non-`bogus`, non-`not-deployed` dependencies (i.e., all dependencies that do exist in the deployed system).  This includes `non-dag` dependencies.
