...
struct Cli {
    ...
    /// The number of samples to emit for each series per poll. Used to simulate
...
impl FromStr for FieldRequest {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        if cardinality == 0 {
            return Err("requested cardinality must not be zero".to_string());
        }
        if let Some(max_cardinality) = max_cardinality(field_type) {
            if cardinality > max_cardinality {
                return Err(format!(
                    "requested cardinality {cardinality} greater than max \
                     cardinality {max_cardinality} for field type {field_type}"
                ));
            }
        }
...
// Get the maximum allowed cardinality for a field type, if the field type has
// cardinality less than the cardinality of the u64 used to represent the
// requested cardinality. Else return None.
fn max_cardinality(field_type: FieldType) -> Option<u64> {
    match field_type {
        FieldType::Bool => Some(2),
        FieldType::I8 | FieldType::U8 => Some(1 << 8),
        FieldType::I16 | FieldType::U16 => Some(1 << 16),
        FieldType::I32 | FieldType::U32 => Some(1 << 32),
        FieldType::I64
        | FieldType::U64
        | FieldType::String
        | FieldType::Uuid
        | FieldType::IpAddr => None,
    }
}
...
impl Producer for LoadTestProducer {
    fn produce(
        ...
        let mut samples = Vec::with_capacity(
            self.series.len() * self.samples_per_series as usize,
        );
        for _ in 0..self.samples_per_series {
