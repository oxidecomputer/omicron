...
async fn test_nexus_add_remove() {
    LiveTestContext::run_test(
        ...
        &|lc: &LiveTestContext| {
            ...
            let planning_input =
                PlanningInputFromDb::assemble(&opctx, &datastore)
                    .await
                    .expect("planning input");
            let initial_nexus_clients =
                lc.all_internal_nexus_clients().await.unwrap();
            let nexus =
            ...
            let (blueprint1, blueprint2) = blueprint_edit_current_target(
                ...
                &|builder: &mut BlueprintBuilder| {
                    let nnexus = builder.sled_num_running_zones_of_kind(
                        sled_id,
                        ZoneKind::Nexus,
                    );
                    let count = builder
            ...
            let new_zone_client =
                lc.specific_internal_nexus_client(new_zone_sockaddr);
    ...
    .expect("new Nexus to be usable");
            ...
            // Create a demo saga from the new Nexus zone.  We'll use this to
            // test that when the zone is expunged, its saga gets moved to a
            // different Nexus.
            ...
            let sagas_list = list_sagas(&new_zone_client)
                .await
                .expect("new Nexus sagas_list");
    ...
    .unwrap();
            ...
            let nexus_found = wait_for_condition(
                || async {
                    for nexus_client in &initial_nexus_clients {
                        assert!(
                            nexus_client.baseurl() != new_zone_client.baseurl()
                        );
            ...
            // Now, complete the demo saga on whichever instance is running it
            // now.  `saga_demo_complete` is not synchronous.  It just unblocks
            // the saga.  We'll need to poll a bit to wait for it to finish.
            nexus_found
            ...
            let found = wait_for_condition(
                || async {
                    let sagas =
                        list_sagas(&nexus_found).await.expect("listing sagas");
                    debug!(log, "found sagas (last): {:?}", sagas);
                    let found =
                        sagas.into_iter().find(|s| s.id == saga_id).unwrap();
