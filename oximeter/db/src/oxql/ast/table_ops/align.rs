...
mod tests {
    ...
    fn test_mean_delta_value_in_window() {
        ...
        let window_end = now - Duration::from_secs_f64(4.5);
        assert!(
            mean_delta_value_in_window(
                start_times,
                timestamps,
            ...
            .is_none(),
            "This should overlap no intervals"
        );

        let window_start = now - Duration::from_secs(4);
        let window_end = now - Duration::from_secs_f64(3.5);
        ...
        .expect("This should overlap the first interval");
        assert_eq!(
            mean, 0.0,
            "This interval overlaps the first point, with a value of 0.0, \
            and the window overlaps half the window"
        );
        ...
        .expect("This should overlap the second interval");
        assert_eq!(
            mean, 0.5,
            "This interval overlaps the second point, with a value of 1.0, \
            and the window overlaps half the window"
        );

        let window_start = now - Duration::from_secs(3);
        let window_end = now - Duration::from_secs_f64(1.5);
        let mean = mean_delta_value_in_window(
            start_times,
            timestamps,
            input_points,
        ...
        .expect("This should overlap the second and third intervals");
        assert_eq!(
            mean, 2.0,
            "This overlaps the first interval entirely, and half the second. \
            That should have a value of 1.0 and half the second (2.0), for \
            a total of 2.0",
        );
        */
