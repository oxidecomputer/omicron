...
pub enum Error {
    ...
    #[error("Could not read timeseries_to_delete")]
...
// The name of the oximeter cluster, in the case of replicated tables.
//
// This must match what is used in the replicated SQL files when creating the
// replicated database itself, and the XML files describing the cluster.
...
// Name of the table storing database version information.
const VERSION_TABLE_NAME: &str = "version";

// During schema upgrades, it is possible to list timeseries that should be
// deleted, rather than deleting the entire database. These must be listed
// one-per-line, in the file inside the schema version directory with this name.
const TIMESERIES_TO_DELETE_FILENAME: &str = "timeseries-to-delete.txt";
