...
#[error("new Units value (`Hertz`) cannot be represented in old API version")]
pub struct UnitsConversionHertzError;
...
impl TryFrom<Units> for crate::v2::schema::Units {
    type Error = UnitsConversionHertzError;
    ...
    fn try_from(value: Units) -> Result<Self, Self::Error> {
        match value {
            ...
            Units::Joules => Ok(Self::Joules),
            Units::Hertz => Err(UnitsConversionHertzError),
        }
    }
}
...
impl TryFrom<TimeseriesSchema> for crate::v2::schema::TimeseriesSchema {
    type Error = UnitsConversionHertzError;
