...
//! Functional code for the histogram types.
    ...
    T: HistogramSupport,
{
    ...
    ///
    /// The provided bins should be "back-to-back", so that the right edge of a
    /// bin and its rightward neighbor share a boundary. There should be no gaps
    /// and the bins must be strictly increasing. Only finite values are
    /// supported (i.e., not NaN or +/- infinity).
    ///
    /// Note that additional bins on the left and right may be added, to ensure
    /// that the bins extend over the entire support of the histogram.
