...
/// The resource usage of an oximeter timeseries.
#[derive(Clone, Debug, Deserialize, JsonSchema, Serialize)]
pub struct TimeseriesUsage {
    /// The name of the timeseries.
    pub name: String,
    /// The approximate number of bytes consumed on disk.
    pub n_bytes: u64,
    /// The number of samples in the timeseries.
    pub n_samples: u64,
}

impl IdOrdItem for TimeseriesUsage {
    type Key<'a> = &'a String;
...
/// The resource usage of all oximeter timeseries.
#[derive(Clone, Debug, Deserialize, JsonSchema, Serialize)]
pub struct OximeterUsage {
    /// Usage for each timeseries.
    pub timeseries: IdOrdMap<TimeseriesUsage>,
    /// The time at which the usage calculation was started.
    pub started_at: DateTime<Utc>,
    /// The time at which the usage calculation was completed.
    pub completed_at: DateTime<Utc>,
...
/// An error when computing the usage of all oximeter timeseries.
#[derive(Clone, Debug, Deserialize, JsonSchema, Serialize)]
pub struct OximeterUsageError {
    pub timestamp: DateTime<Utc>,
    pub error: String,
}

/// The result of ongoing attempts to compute oximeter timeseries usage.
#[derive(Clone, Debug, Default, Deserialize, JsonSchema, Serialize)]
pub struct OximeterUsageResult {
    /// The last successful result computing the usage.
    pub last_success: Option<OximeterUsage>,
    /// The last error when computing the usage.
    pub last_error: Option<OximeterUsageError>,
}
