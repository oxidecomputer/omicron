...
impl UpdatePane {
    ...
    fn update_component_list_items(
        ...
        if let UpdateItemState::Running { event_report } =
...
impl ComponentUpdateListState {
    fn process_report(&mut self, report: EventReport) {
        ...
        for (step_key, value) in event_buffer.steps() {
            let step_info = value.step_info();
        ...
        let selected_needs_reset = match self.selected {
            Some(step_key) => {
                // If step_keys doesn't contain the selected step key, means
                // that the step key disappeared (which should never happen
                // unless wicketd decided to send us a brand new
                // EventReport).
